  ...
  const submit = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("create-user", {
      body: {
        ...
        // selfServiceEnabled defaults to true: a new SP is created with no skills or
        // coverage set, and the invite flow (the recommended option above) tells them
        // to finish their own profile after setting a password. Self-service being
        // hardcoded off here meant every new SP's Settings > Skills/Coverage was
        // permanently locked ("View only — ask your admin") with literally nothing
        // filled in, so there was no way for them to actually do that -- an admin
        // still has to fill it in for them. Defaulting on lets them complete it
        // themselves right after signup, same as before; an admin can still switch it
        // off per-SP afterwards in their profile editor if tighter control is wanted.
