...
function WorkloadWidget({tabs}){
  ...
  // Nothing left to do anywhere -- swap the tab list for a single
  // congratulatory banner instead of showing three empty tabs. "View"
  // drops back into the normal tabbed layout (still all zero counts) for
  // anyone who wants to double-check rather than just take the banner's
  // word for it.
  if(totalItems===0&&!expanded){
    return (
      <div style={{background:C.successLight,border:`1.5px solid ${C.success}`,borderRadius:12,padding:"13px 18px",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:22,height:22,borderRadius:6,background:C.success,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
          ...
          <span style={{fontWeight:800,color:C.success,fontSize:14}}>All tasks complete!</span>
        </div>
        <button onClick={()=>setExpanded(true)} style={{background:"none",border:"none",color:C.success,opacity:.8,fontWeight:700,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>View</button>
      </div>
    );
  }

  // Expanded from the banner above with everything still at zero -- keep
  // the green "all clear" theme going rather than switching back to the
  // neutral grey StatusTabs pills, so it reads as "still all clear, here's
  // the breakdown" rather than looking like there's suddenly work again.
  if(totalItems===0&&expanded){
    return (
      <div style={{background:C.successLight,border:`1.5px solid ${C.success}`,borderRadius:12,padding:"13px 17px",marginBottom:14}}>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {tabs.map(t=>{
            const isActive = t.key===activeTab.key;
            return (
              <button key={t.key} onClick={()=>setActive(t.key)} style={{
                border:isActive?"none":`1.5px solid ${C.success}`,borderRadius:20,padding:"7px 15px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",
                display:"flex",alignItems:"center",gap:7,
                background:isActive?C.success:"transparent",
                color:isActive?"#000000":C.success,
              }}>
                {t.label}
                <span style={{background:isActive?"rgba(0,0,0,0.15)":"rgba(74,222,128,0.15)",color:isActive?"#000000":C.success,borderRadius:10,padding:"1px 7px",fontSize:11,fontWeight:800}}>{t.items.length}</span>
              </button>
            );
          })}
        </div>
        <div style={{marginTop:14,textAlign:"center",color:C.success,opacity:.8,fontSize:13}}>Nothing here — all caught up.</div>
      </div>
    );
  }

  // Still got outstanding queues -- box stays the plain neutral card that
  // matches the rest of the dashboard; only the tab pills themselves turn
  // amber/orange to flag "this needs attention" (vs. the all-clear state
  // above, which recolours the whole widget green). Individual row content
  // (PBadge, £ amounts, etc.) keeps its own colour-coding as before.
  return (
    <div style={{background:C.card,borderRadius:12,boxShadow:"0 1px 3px rgba(0,0,0,.05)",overflow:"hidden",marginBottom:14}}>
      <div style={{padding:"13px 17px 0",display:"flex",gap:8,flexWrap:"wrap",marginBottom:14}}>
        {tabs.map(t=>{
          const isActive = t.key===activeTab.key;
          return (
            <button key={t.key} onClick={()=>setActive(t.key)} style={{
              border:"none",borderRadius:20,padding:"7px 15px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",
              display:"flex",alignItems:"center",gap:7,
              background:isActive?C.warn:C.warnLight,
              color:isActive?"#000000":C.warn,
            }}>
              {t.label}
              <span style={{background:isActive?"rgba(0,0,0,0.15)":"rgba(251,191,36,0.2)",color:isActive?"#000000":C.warn,borderRadius:10,padding:"1px 7px",fontSize:11,fontWeight:800}}>{t.items.length}</span>
            </button>
          );
        })}
      </div>
