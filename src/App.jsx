...
function SettingsView({pricing,onReload,users,onUserCreated,currentUserId,usersTab,onUsersTabChange,onArchive,onRestore,onDelete,onResetPassword}){
  ...
  const [tab,setTab] = useState("pricing"); // "pricing" | "users" -- subsections of Settings
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:18}}>
        {[["pricing","Pricing"],["users","Users"]].map(([key,label])=>(
          <button key={key} onClick={()=>setTab(key)} style={{border:"none",borderRadius:7,padding:"7px 15px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",background:tab===key?C.primary:"rgba(255,255,255,0.07)",color:tab===key?"#000":C.mid}}>{label}</button>
        ))}
      </div>
...
export default function App() {
  ...
  const NAV = [{id:"dashboard",label:"Dashboard",ic:"⊞"},{id:"jobs",label:"Bookings",ic:"📋"},{id:"engineers",label:"SP's",ic:"🔧"},{id:"payments",label:"Payments",ic:"£"},...(isOwner?[{id:"settings",label:"Settings",ic:"⚙️"}]:[])];
    ...
    <div style={{fontFamily:"'Inter','Segoe UI',sans-serif",background:"#000000",color:C.text,minHeight:"100vh",display:"flex"}}>
      ...
      <div className="pb-[88px] md:pb-[22px]" style={{flex:1,overflowY:"auto",paddingTop:22,paddingLeft:24,paddingRight:24}}>
        <div style={{maxWidth:1080}}>
          ...
          {/* ── SETTINGS (Pricing + Users subsection) ── */}
