    ...
    <div>
      ...
      <div style={{background:C.primaryLight,border:`1px solid #BFDBFE`,borderRadius:9,padding:"12px 14px",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:10}}>
          <button type="button" onClick={()=>set("isExternal",false)} style={{flex:1,border:"none",borderRadius:7,padding:"7px 0",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",background:!f.isExternal?C.primary:"rgba(255,255,255,0.07)",color:!f.isExternal?"#000":C.mid}}>In-house Engineer</button>
          <button type="button" onClick={()=>set("isExternal",true)} style={{flex:1,border:"none",borderRadius:7,padding:"7px 0",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",background:f.isExternal?C.primary:"rgba(255,255,255,0.07)",color:f.isExternal?"#000":C.mid}}>External Engineer (one-off)</button>
        </div>
        {f.isExternal ? (
          <div>
            <div style={{fontSize:11,color:C.mid,lineHeight:1.5,marginBottom:10}}>
              Use this when no in-house engineer covers this job's area — bring in a one-time external engineer for just this booking. Their details are kept with this job only; they won't get a Portal login or appear on the Engineers page.
            </div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
              <Fl label="Engineer / Contractor Name"><input style={inp} value={f.externalName} onChange={e=>set("externalName",e.target.value)} placeholder="Full name"/></Fl>
  ...
  return (
    <Modal title={`Reassign — Job #${job.id}: ${job.customer}`} onClose={onClose}>
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        <button type="button" onClick={()=>setMode("inhouse")} style={{flex:1,border:"none",borderRadius:7,padding:"7px 0",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",background:mode==="inhouse"?C.primary:"rgba(255,255,255,0.07)",color:mode==="inhouse"?"#000":C.mid}}>In-house Engineer</button>
        <button type="button" onClick={()=>setMode("external")} style={{flex:1,border:"none",borderRadius:7,padding:"7px 0",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",background:mode==="external"?C.primary:"rgba(255,255,255,0.07)",color:mode==="external"?"#000":C.mid}}>External Engineer (one-off)</button>
      </div>

      {mode==="external" ? (
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,color:C.mid,lineHeight:1.5,marginBottom:10}}>No in-house engineer for this one? Bring in a one-time external engineer for just this booking.</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <Fl label="Engineer / Contractor Name"><input style={inp} value={ext.name} onChange={e=>setExtField("name",e.target.value)} placeholder="Full name"/></Fl>
...
export default function App() {
  ...
  const unassigned = jobs.filter(j=>!j.engineerId&&!j.isExternal&&!["Cancelled","Completed","Beyond Repair"].includes(j.status));
    ...
    <div style={{fontFamily:"'Inter','Segoe UI',sans-serif",background:"#000000",color:C.text,minHeight:"100vh",display:"flex"}}>
        ...
        <div style={{maxWidth:1080}}>
          ...
          {view==="dashboard"&&(
            <div>
              ...
              <div style={{display:"flex",gap:12,marginBottom:18,flexWrap:"wrap"}}>
                ...
                <StatCard label="Active Bookings" value={jobs.filter(j=>!["Cancelled","Completed","Beyond Repair"].includes(j.status)).length} color={C.blue}/>
          ...
          {view==="jobs"&&(
            <div>
              ...
              <div style={{display:"flex",gap:9,marginBottom:12,flexWrap:"wrap"}}>
                ...
                <select style={{...inp,maxWidth:165}} value={fsEng} onChange={e=>setFsEng(e.target.value)}><option value="All">All Engineers</option><option value="External">External (one-off)</option>{engineers.map(e=><option key={e.id} value={e.id}>{e.name}</option>)}</select>
              </div>
          ...
          {view==="engineers"&&(()=>{
            ...
            <div>
              ...
              {engTab==="archived" ? (
                <div style={{background:C.card,borderRadius:13,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
                  {shown.length===0&&<div style={{padding:22,textAlign:"center",color:C.light,fontSize:13}}>No archived engineers.</div>}
                  {shown.map((eng,i)=>(
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(330px,1fr))",gap:14}}>
                {shown.length===0&&<div style={{color:C.light,fontSize:13}}>{engTab==="onboarding"?"No engineers currently onboarding.":"No engineers yet — add one to get started."}</div>}
                {shown.map(eng=>{
          ...
          {view==="payments"&&(
            <div>
              ...
              {externalGroups.length>0&&(
                <>
                  <div style={{fontSize:12,fontWeight:700,color:C.mid,textTransform:"uppercase",letterSpacing:.6,margin:"20px 0 10px"}}>External Engineers (one-off jobs)</div>
                  {externalGroups.map((g,gi)=>{
                    const done=g.jobs.filter(j=>j.status==="Completed");
                    const unpaid=done.filter(j=>!j.paid);
                    const paid=done.filter(j=>j.paid);
                    return (
                      <div key={gi} style={{background:C.card,borderRadius:13,boxShadow:"0 1px 3px rgba(0,0,0,.05)",marginBottom:16,overflow:"hidden",border:`1px solid ${C.purple}33`}}>
                        <div style={{padding:"13px 18px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:"#161B22"}}>
                          <div style={{display:"flex",gap:10,alignItems:"center"}}>
                            <Av initials={g.name.split(" ").map(w=>w[0]).join("").slice(0,2)} color={C.purple}/>
                            <div>
                              <div style={{fontWeight:800,fontSize:14,display:"flex",alignItems:"center",gap:6}}>{g.name}<span style={{background:C.purpleLight,color:C.purple,padding:"1px 7px",borderRadius:10,fontSize:9,fontWeight:800}}>EXTERNAL</span></div>
                              <div style={{color:C.light,fontSize:11}}>{[g.company,g.phone].filter(Boolean).join(" · ")||"No contact details"} · {done.length} completed</div>
                            </div>
                          </div>
                          <div style={{textAlign:"right"}}><div style={{color:C.danger,fontWeight:900,fontSize:16}}>£{unpaid.reduce((s,j)=>s+Number(j.rate||0),0)} owed</div><div style={{color:C.success,fontSize:11,fontWeight:600}}>£{paid.reduce((s,j)=>s+Number(j.rate||0),0)} paid total</div></div>
                        </div>
          ...
          {/* ── USERS ── */}
          {view==="users"&&isOwner&&(
            <UserManager users={users} onUserCreated={loadStaffUsers} currentUserId={currentUser.id}
              tab={usersTab} onTabChange={setUsersTab}
      ...
      {(showNew||editJob)&&(
        <Modal title={editJob?`Edit Job #${editJob.id}`:"New Booking"} onClose={()=>{setShowNew(false);setEditJob(null);}} wide>
          <JobForm initial={editJob} onSave={saveJob} onCancel={()=>{setShowNew(false);setEditJob(null);}} canEditRate={isOwner} engineers={assignableEngineers} jobs={jobs}/>
        </Modal>
      )}
      ...
      {confirmModal&&(
        <ConfirmModal
          ...
          message={confirmModal.kind==="archive"
            ...
            : "This permanently deletes their login, profile, and (if an engineer) their engineer record. This cannot be undone."}
