  ...
  const submit = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("create-user", {
      body: {
        ...
        engineer:{ payRate:Number(f.rate)||45, type:f.type, postcodes:[], applianceTypes:[], brandExclusions:{}, selfServiceEnabled:false },
      },
...
export default function App() {
  ...
  const todayNewBookings = jobs.filter(j=>j.createdAt && localDateStr(new Date(j.createdAt))===TODAY);
          ...
          {view==="dashboard"&&(
            <div>
              ...
              <div style={{display:"flex",gap:12,marginBottom:18,flexWrap:"wrap"}}>
                <StatCard label="Today's Bookings" value={todayNewBookings.length} color={C.primary} sub={`${todayNewBookings.filter(j=>j.status==="Completed").length} done`}/>
