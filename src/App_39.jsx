...
// The different kinds of SP (Service Provider) an engineers row can be --
// see the `type` column added on public.engineers in
// 0005_engineer_types.sql. "Boiler Engineers" is a placeholder for now (no
// real SPs of that type yet), added so the side nav/badge on the SP's page
// already reads correctly the day the first one is created. Keep this list
// and the migration's check constraint in sync if a type is ever added.
const SP_TYPES = [
  {key:"appliance", label:"Appliance Engineers"},
  {key:"boiler",    label:"Boiler Engineers"},
];
...
//
// localDateStr() is the same local-date computation, factored out so it can
// also be applied to a booking's createdAt timestamp (see todayNewBookings
// below) -- createdAt comes back from Postgres as a UTC timestamptz, so
// comparing its raw string against TODAY would be wrong for anyone booked
// late evening/early morning local time; converting both sides via `new
// Date(...)` first keeps the comparison in the browser's local day.
...
// For real timestamptz values (e.g. job.createdAt) -- these already carry a real
// time + timezone from Postgres, unlike the plain YYYY-MM-DD strings `fmt` above
// is for (scheduledDate/completedDate), so this must NOT append a fake T12:00:00.
...
// Strips a phone number down to what a tel: link needs (leading + kept,
// everything else that isn't a digit dropped) -- the on-screen text keeps
// whatever formatting was entered (spaces, etc).
const telHref = phone => "tel:" + String(phone||"").trim().replace(/(?!^\+)[^\d]/g,"");

// ─── ATOMS ───────────────────────────────────────────────────────────────────
// Click-to-call phone number. Renders as a tappable link (native "tel:"
// handler) so it works from a desktop browser and from mobile without any
// extra setup. This is the one place phone numbers become clickable, so a
// future phone-system/CTI integration (e.g. swapping the tel: link for a
// click-to-dial API call) only needs to change this component.
const CallLink = ({phone,style}) => {
  if(!phone) return null;
  return (
    <a href={telHref(phone)} onClick={e=>e.stopPropagation()} title={`Call ${phone}`}
...
// Flags a booking or SP created by Settings -> System -> Test (see
// isTest on mapBookingRow/mapEngineerRow) so it's never mistaken for a real
// customer or engineer at a glance.
const TestPill = () =>
  <span style={{background:"rgba(251,191,36,0.15)",color:"#fbbf24",padding:"1px 7px",borderRadius:10,fontSize:9,fontWeight:800,letterSpacing:.5,textTransform:"uppercase"}}>Test</span>;
// Flags a booking that opted into "Repair + Protection Plan" (see
// wantsProtectionPlan on mapBookingRow) wherever a booking row is shown in
// a list, next to TestPill -- same shape, different colour so the two are
// never confused at a glance.
const ProtectionPlanPill = () =>
  <span style={{background:C.purpleLight,color:C.purple,padding:"1px 7px",borderRadius:10,fontSize:9,fontWeight:800,letterSpacing:.5,textTransform:"uppercase"}}>🛡 Plan</span>;
...
// Optional onClick turns the card into a shortcut (Dashboard cards jump to
// the matching page/filter) -- hover lift + keyboard (Enter/Space) support.
const StatCard = ({label,value,sub,color="#d4ff3c",onClick,title}) =>
  <div onClick={onClick} title={title}
    role={onClick?"button":undefined} tabIndex={onClick?0:undefined}
    onKeyDown={onClick?(e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();onClick();}}):undefined}
    onMouseOver={onClick?(e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow=`0 6px 20px rgba(0,0,0,.4), 0 0 0 1px ${color}55`;}):undefined}
    onMouseOut={onClick?(e=>{e.currentTarget.style.transform="none";e.currentTarget.style.boxShadow="0 4px 16px rgba(0,0,0,.3)";}):undefined}
    style={{background:C.card,borderRadius:11,padding:"16px 18px",borderLeft:`4px solid ${color}`,flex:1,minWidth:120,boxShadow:"0 4px 16px rgba(0,0,0,.3)",cursor:onClick?"pointer":"default",transition:"transform .12s, box-shadow .12s"}}><div style={{fontSize:24,fontWeight:900,color:C.text}}>{value}</div><div style={{fontSize:11,color:C.mid,marginTop:1}}>{label}</div>{sub&&<div style={{fontSize:10,color,fontWeight:700,marginTop:2}}>{sub}</div>}</div>;
...
const IconShield = ({size=18}) => ( // Protection Plans
  <svg width={size} height={size} {...navIconBase}>
...
// ─── MODAL ───────────────────────────────────────────────────────────────────
// `accent` is an opt-in thin border (matching C.primary, the brand lime/green)
// for modals that get lost against the near-black backdrop -- e.g. Add SP,
// where the plain #141414-on-black card had no visible edge at all. Left off
// by default so it doesn't change every other modal in the app.
const Modal = ({title,onClose,children,wide,accent}) => (
  <div style={{position:"fixed",inset:0,background:"rgba(10,15,30,.6)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:12}} onClick={onClose}>
    <div style={{background:C.card,borderRadius:14,width:"100%",maxWidth:wide?840:620,maxHeight:"93vh",overflowY:"auto",border:accent?`1px solid ${C.primary}`:"1px solid transparent",boxShadow:accent?`0 24px 80px rgba(0,0,0,.3), 0 0 18px rgba(212,255,60,0.15)`:"0 24px 80px rgba(0,0,0,.3)"}} onClick={e=>e.stopPropagation()}>
...
function StatusTabs({tabs,value,onChange,activeColor=C.primary}){
  return (
    <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
      {tabs.map(t=>(
        <button key={t.key} onClick={()=>onChange(t.key)} style={{
          ...
          background:value===t.key?activeColor:"rgba(255,255,255,0.07)",
...
// "What needs addressing" widget for the Dashboard -- tabbed like the
// Active/Onboarding/Archive pills above (reuses StatusTabs), but each tab
// is a different queue blocking a booking (or an SP's onboarding) from
// moving forward, so staff can jump straight to whichever queue is piling
// up instead of scanning the full Bookings/SP's tables. `tabs` is
// [{key,label,items:[{key,title,sub,right,onClick}]}] -- callers build the
// item list (and its click target) per tab since a "job" row and an
// "engineer" row show different fields.
function WorkloadWidget({tabs}){
  const [active,setActive] = useState(tabs[0]?.key);
  const activeTab = tabs.find(t=>t.key===active) || tabs[0];
  if(!activeTab) return null;
  const totalItems = tabs.reduce((s,t)=>s+t.items.length,0);

  // Nothing left to do anywhere -- same plain card as the busy state below
  // (no separate tinted/bordered treatment, no expand-to-tabs interaction
  // -- that was more going on than the "all clear" case needs), just a
  // green checkmark row swapped in for the tabs.
  if(totalItems===0){
    return (
      <div style={{background:C.card,borderRadius:12,boxShadow:"0 1px 3px rgba(0,0,0,.05)",overflow:"hidden",marginBottom:14}}>
        <div style={{padding:"13px 17px",display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:22,height:22,borderRadius:6,background:"#D5FE54",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
            <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#000000" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
          </div>
          <span style={{fontWeight:800,color:"#D5FE54",fontSize:14}}>All tasks complete!</span>
        </div>
        <div style={{borderTop:`1px solid ${C.border}`,padding:22,textAlign:"center",color:C.light,fontSize:13}}>Nothing here — all caught up.</div>
      </div>
    );
  }

  // Still got outstanding queues -- box stays the plain neutral card that
  // matches the rest of the dashboard, same grey inactive pills as every
  // other StatusTabs row in the app; only the *active* pill turns
  // amber/orange (instead of the usual lime) to flag "this needs
  // attention" vs. the all-clear state above, which recolours everything
  // green. Individual row content (PBadge, £ amounts, etc.) keeps its own
  // colour-coding as before.
  return (
    <div style={{background:C.card,borderRadius:12,boxShadow:"0 1px 3px rgba(0,0,0,.05)",overflow:"hidden",marginBottom:14}}>
      <div style={{padding:"13px 17px 0"}}>
        <StatusTabs value={activeTab.key} onChange={setActive} tabs={tabs.map(t=>({key:t.key,label:t.label,count:t.items.length}))} activeColor={C.warn}/>
      </div>
      <div style={{borderTop:`1px solid ${C.border}`}}>
        {activeTab.items.length===0
          ? <div style={{padding:22,textAlign:"center",color:C.light,fontSize:13}}>Nothing here — all caught up.</div>
          : activeTab.items.map(item=>(
            <div key={item.key} onClick={item.onClick} style={{padding:"10px 17px",borderBottom:"1px solid #262626",cursor:item.onClick?"pointer":"default",display:"flex",alignItems:"center",gap:12}}
              onMouseOver={e=>{if(item.onClick)e.currentTarget.style.background="#1E2530";}} onMouseOut={e=>e.currentTarget.style.background="transparent"}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontWeight:700,fontSize:13}}>{item.title}</div>
                {item.sub&&<div style={{color:C.light,fontSize:11,marginTop:1}}>{item.sub}</div>}
              </div>
              {item.right}
            </div>
          ))}
...
// ─── JOB FORM ─────────────────────────────────────────────────────────────────
function JobForm({initial,onSave,onCancel,canEditRate,engineers,jobs,externalSps=[],defaultStatus="Booked"}) {
  const blank = {customer:"",phone:"",email:"",address:"",postcode:"",appliance:APPLIANCE_TYPES[0],brand:"",applianceAge:"",issue:"",source:SOURCES[0],status:defaultStatus,engineerId:null,scheduledDate:"",scheduledTime:"09:00",completedDate:"",priority:"Normal",partsNeeded:false,partsOrdered:false,partsArrived:false,rate:"",paid:false,notes:"",isExternal:false,externalEngineerId:null,externalName:"",externalPhone:"",externalEmail:"",externalCompany:"",wantsProtectionPlan:false};
  ...
  return (
    <div>
      ...
      {/* Whether the customer chose "Repair + Protection Plan" over repair
          only -- normally set automatically from the bookarepair2 form via
          the submit-booking Edge Function, but staff can tick/untick this
          for a booking taken over the phone or entered by hand. Ticking it
          creates a row on the Protection Plans page the moment this booking
          is saved (see 0006_service_plans.sql); unticking it later does NOT
          retroactively cancel that plan -- use Cancel Plan on that page for
          that, same as a booking's own Cancelled status not being deletion. */}
      <label style={{display:"flex",alignItems:"center",gap:9,background:C.purpleLight,border:`1px solid ${C.purple}`,borderRadius:9,padding:"10px 14px",marginBottom:12,cursor:"pointer"}}>
        <input type="checkbox" checked={!!f.wantsProtectionPlan} onChange={e=>set("wantsProtectionPlan",e.target.checked)}/>
        <span style={{fontSize:13,fontWeight:700,color:C.purple}}>🛡 Repair + Protection Plan (covers future callouts on this appliance)</span>
      </label>
        ...
        {f.isExternal ? (
          <div>
            <div style={{fontSize:11,color:C.mid,lineHeight:1.5,marginBottom:10}}>
              Use this when no in-house SP covers this job's area — bring in an external SP for just this booking, from your External SP's roster or as a fresh one-off. Either way they won't get a Portal login or appear under In-house SPs.
            </div>
            {externalSps.length>0&&(
              <Fl label="External SP">
                <select style={inp} value={f.externalEngineerId||""} onChange={e=>{
                  const id=e.target.value;
                  if(!id){ set("externalEngineerId",null); return; }
                  const s=externalSps.find(x=>x.id===id);
                  setF(p=>({...p,externalEngineerId:id,externalName:s?.name||"",externalPhone:s?.phone||"",externalEmail:s?.email||"",externalCompany:s?.company||"",rate:(!p.rate&&canEditRate&&s?.rate!=null)?s.rate:p.rate}));
                }}>
                  <option value="">— One-off (type details below) —</option>
                  {externalSps.map(s=><option key={s.id} value={s.id}>{s.name}{s.postcodes.length?` (${s.postcodes.join(", ")})`:""}</option>)}
                </select>
              </Fl>
            )}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
...
// Read-only display of the Portal's Engineer Notes for this job -- a
// running, timestamped log an engineer can add to from their side (see
// portal/src/App.jsx's BookingDetailModal), stored in its own `booking_notes`
// table rather than a column on `bookings` since it's an append-only list of
// entries, not a single field either side overwrites. This is separate from
// `job.notes` just below it, which is FixFlow's own single staff-only
// "Internal Notes" field -- staff can't add to this log from here, only read
// it, so there's no write UI, just a fetch keyed on the job id.
function EngineerNotes({ jobId }) {
  ...
  return (
    <div style={{background:"#1E2530",borderRadius:9,padding:12,marginBottom:12}}>
      <div style={{fontSize:10,color:C.light,fontWeight:700,textTransform:"uppercase",marginBottom:8}}>Engineer Notes</div>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {notes.map(n=>(
          <div key={n.id}>
            <div style={{fontSize:11,color:C.light,marginBottom:2}}>{n.author_name} · {fmtDateTime(n.created_at)}</div>
  ...
  return (
    <div>
      <div style={{display:"flex",gap:7,marginBottom:14,flexWrap:"wrap"}}>
        ...
        {job.wantsProtectionPlan&&<span style={{background:C.purpleLight,color:C.purple,padding:"2px 8px",borderRadius:4,fontSize:10,fontWeight:800}}>🛡 PROTECTION PLAN</span>}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
        <div style={{background:"#1E2530",borderRadius:9,padding:12}}>
          ...
          <div style={{color:C.mid,fontSize:12,marginTop:2}}><CallLink phone={job.phone}/></div>
        ...
        <div style={{background:"#1E2530",borderRadius:9,padding:12}}>
          ...
          {job.isExternal ? (
            <>
              ...
              {job.externalPhone&&<div style={{color:C.mid,fontSize:12,marginTop:2}}><CallLink phone={job.externalPhone}/></div>}
              {job.externalEmail&&<div style={{color:C.mid,fontSize:12}}>✉ {job.externalEmail}</div>}
            </>
          ) : eng?<><div style={{fontWeight:700,fontSize:14}}>{eng.name}</div><div style={{color:C.mid,fontSize:12,marginTop:2}}><CallLink phone={eng.phone}/></div></>
...
function ReassignModal({job,engineers,jobs,externalSps=[],onReassign,onClose}) {
  const [mode,setMode] = useState(job.isExternal?"external":"inhouse");
  const [chosen,setChosen] = useState(job.engineerId||"");
  const [ext,setExt] = useState({id:job.externalEngineerId||null,name:job.externalName||"",phone:job.externalPhone||"",email:job.externalEmail||"",company:job.externalCompany||""});
  const setExtField = (k,v)=>setExt(p=>({...p,[k]:v}));
  const pickExternalSp = (id) => {
    if(!id){ setExt({id:null,name:"",phone:"",email:"",company:""}); return; }
    const s=externalSps.find(x=>x.id===id);
    setExt({id, name:s?.name||"", phone:s?.phone||"", email:s?.email||"", company:s?.company||""});
  };
    ...
    <Modal title={`Reassign — Job ${job.bookingRef||('#'+job.id)}: ${job.customer}`} onClose={onClose}>
      ...
      {mode==="external" ? (
        <div style={{marginBottom:14}}>
          <div style={{fontSize:11,color:C.mid,lineHeight:1.5,marginBottom:10}}>No in-house SP for this one? Pick one from your External SP's roster, or bring in a fresh one-off for just this booking.</div>
          {externalSps.length>0&&(
            <Fl label="External SP">
              <select style={inp} value={ext.id||""} onChange={e=>pickExternalSp(e.target.value)}>
                <option value="">— One-off (type details below) —</option>
                {externalSps.map(s=><option key={s.id} value={s.id}>{s.name}{s.postcodes.length?` (${s.postcodes.join(", ")})`:""}</option>)}
              </select>
            </Fl>
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        ...
        <Fl label="SP Type">
          <select style={inp} value={e.type||"appliance"} onChange={ev=>setEng("type",ev.target.value)}>
...
// ─── EXTERNAL SP EDITOR — full coverage/document profile for a roster
// entry (see 0007_external_sps.sql). Structurally the same idea as
// EngineerEditor above (postcodes, appliance types + brand exclusions,
// ID/insurance documents) but flatter: no Portal login, so no self-service
// toggle, working hours, or onboarding checklist. AddExternalSpModal below
// creates the bare row (name/phone/email/company/rate); staff then use this
// editor to fill in coverage and documents, same two-step flow as
// AddEngineerModal -> EngineerEditor. ───────────────────────────────────────
function ExternalSpEditor({sp,onSave,onCancel,isOwner}) {
  const [e,setE] = useState({...sp,brandExclusions:{...APPLIANCE_TYPES.reduce((a,t)=>({...a,[t]:[]}),{}),...sp.brandExclusions}});
  const [newPc,setNewPc] = useState("");
  const [uploading,setUploading] = useState(null);
  const [uploadErr,setUploadErr] = useState("");
  const [viewingDoc,setViewingDoc] = useState(null);
  const [viewErr,setViewErr] = useState("");
  const setSp=(k,v)=>setE(p=>({...p,[k]:v}));
  const addPc=()=>{const pc=newPc.trim().toUpperCase();if(pc&&!e.postcodes.includes(pc))setSp("postcodes",[...e.postcodes,pc]);setNewPc("");};
  const remPc=pc=>setSp("postcodes",e.postcodes.filter(x=>x!==pc));
  const toggleApp=a=>setSp("applianceTypes",e.applianceTypes.includes(a)?e.applianceTypes.filter(x=>x!==a):[...e.applianceTypes,a]);
  const toggleExcl=(app,brand)=>{const cur=e.brandExclusions[app]||[];setSp("brandExclusions",{...e.brandExclusions,[app]:cur.includes(brand)?cur.filter(b=>b!==brand):[...cur,brand]});};

  // Documents live in the same private "engineer-documents" bucket in-house
  // SPs use, just under an "external/<id>/..." path -- the bucket's
  // staff/owner-manage-all storage policy is bucket-wide, not folder-scoped,
  // so no separate bucket or policy is needed for external SPs.
  const uploadDoc = async (kind, file) => {
    if(!file) return;
    setUploadErr(""); setUploading(kind.key);
    const ext = file.name.split(".").pop()||"pdf";
    const path = `external/${sp.id}/${kind.key}-document.${ext}`;
    const { error: upErr } = await supabase.storage.from("engineer-documents").upload(path, file, {upsert:true});
    if(upErr){ setUploadErr(upErr.message); setUploading(null); return; }
    const nowIso = new Date().toISOString();
    const { error: dbErr } = await supabase.from("external_engineers").update({[kind.column]:path,[kind.uploadedColumn]:nowIso}).eq("id",sp.id);
    setUploading(null);
    if(dbErr){ setUploadErr(dbErr.message); return; }
    setSp(kind.pathField, path); setSp(kind.uploadedField, nowIso);
  };
  const viewDoc = async (kind) => {
    const path = e[kind.pathField];
    if(!path) return;
  ...
  return (
    <div>
      <div style={{fontWeight:800,fontSize:13,color:C.mid,marginBottom:12,paddingBottom:8,borderBottom:`1px solid ${C.border}`,textTransform:"uppercase",letterSpacing:.5}}>Contact Details</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        <Fl label="Full Name"><input style={inp} value={e.name} onChange={ev=>setSp("name",ev.target.value)}/></Fl>
        <Fl label="Phone"><input style={inp} value={e.phone||""} onChange={ev=>setSp("phone",ev.target.value)}/></Fl>
        <Fl label="Email"><input style={inp} value={e.email||""} onChange={ev=>setSp("email",ev.target.value)}/></Fl>
        <Fl label="Company"><input style={inp} value={e.company||""} onChange={ev=>setSp("company",ev.target.value)} placeholder="e.g. sole trader / agency name"/></Fl>
        {isOwner&&<Fl label="Pay Rate (£ per job)"><input type="number" style={inp} value={e.rate||""} onChange={ev=>setSp("rate",ev.target.value===""?null:Number(ev.target.value))}/></Fl>}
      </div>

      <div style={{fontWeight:800,fontSize:13,color:C.mid,marginBottom:10,paddingBottom:8,borderBottom:`1px solid ${C.border}`,textTransform:"uppercase",letterSpacing:.5}}>Postcode Areas Covered</div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
        {e.postcodes.map(pc=>(
          <span key={pc} style={{background:C.purpleLight,color:C.purple,padding:"3px 9px",borderRadius:6,fontSize:12,fontWeight:700,display:"flex",alignItems:"center",gap:4}}>
            {pc}
            <button onClick={()=>remPc(pc)} style={{background:"none",border:"none",cursor:"pointer",color:C.purple,fontSize:14,padding:0,lineHeight:1}}>×</button>
          </span>
        ))}
        {!e.postcodes.length&&<span style={{color:C.light,fontSize:12,fontStyle:"italic"}}>No areas added yet</span>}
      </div>
      ...
      <div style={{fontWeight:800,fontSize:13,color:C.mid,marginBottom:10,paddingBottom:8,borderBottom:`1px solid ${C.border}`,textTransform:"uppercase",letterSpacing:.5,marginTop:20}}>Documents</div>
      {uploadErr&&<div style={{background:"rgba(248,113,113,0.15)",color:"#f87171",borderRadius:7,padding:"8px 12px",fontSize:13,marginBottom:10,fontWeight:600}}>{uploadErr}</div>}
      {viewErr&&<div style={{background:"rgba(248,113,113,0.15)",color:"#f87171",borderRadius:7,padding:"8px 12px",fontSize:13,marginBottom:10,fontWeight:600}}>{viewErr}</div>}
      <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:14}}>
        {ENG_DOC_KINDS.map(kind=>{
      ...
      <Fl label="Insurance Expiry Date"><input type="date" style={inp} value={e.insuranceExpiryDate||""} onChange={ev=>setSp("insuranceExpiryDate",ev.target.value)}/></Fl>
      <Fl label="Notes"><textarea style={ta} value={e.notes||""} onChange={ev=>setSp("notes",ev.target.value)} placeholder="Anything else worth remembering about this contractor..."/></Fl>
...
// ─── ADD EXTERNAL SP MODAL — creates the bare external_engineers row (no
// auth account, no Edge Function involved -- there's nothing privileged
// about inserting a plain roster row for staff/owner). Mirrors
// AddEngineerModal's two-step pattern: this captures just contact info +
// rate, then staff use ExternalSpEditor to fill in coverage and documents.
function AddExternalSpModal({onCreated,onCancel}) {
  const [f,setF] = useState({name:"",phone:"",email:"",company:"",rate:""});
  ...
  const submit = async () => {
    ...
    setBusy(true);
    const { data, error } = await supabase.from("external_engineers").insert(externalSpContactFieldsToRow(f)).select().single();
    setBusy(false);
    if(error){ setErr(error.message||"Something went wrong adding this external SP."); return; }
    onCreated(mapExternalRow(data));
  };

  return (
    <Modal title="Add External SP" onClose={onCancel}>
      {err&&<div style={{background:"rgba(248,113,113,0.15)",color:"#f87171",borderRadius:7,padding:"8px 12px",fontSize:13,marginBottom:12,fontWeight:600}}>{err}</div>}
      <div style={{fontSize:12,color:C.mid,lineHeight:1.5,marginBottom:14}}>
        For a contractor outside your normal roster who takes one-off jobs. No Portal login is created -- add postcode coverage, appliance skills and documents next, after this record exists.
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        <Fl label="Full Name"><input style={inp} value={f.name} onChange={e=>set("name",e.target.value)}/></Fl>
        <Fl label="Phone"><input style={inp} value={f.phone} onChange={e=>set("phone",e.target.value)} placeholder="07..."/></Fl>
        <Fl label="Email"><input style={inp} value={f.email} onChange={e=>set("email",e.target.value)}/></Fl>
        <Fl label="Company"><input style={inp} value={f.company} onChange={e=>set("company",e.target.value)} placeholder="e.g. sole trader / agency name"/></Fl>
        <Fl label="Pay Rate (£ per job)"><input type="number" style={inp} value={f.rate} onChange={e=>set("rate",e.target.value)}/></Fl>
      </div>
      <div style={{display:"flex",gap:10,marginTop:10}}>
        <Btn onClick={submit} full style={{padding:"10px 0",opacity:busy?.7:1}}>{busy?"Adding…":"Add External SP"}</Btn>
        <Btn onClick={onCancel} variant="ghost" full style={{padding:"10px 0"}}>Cancel</Btn>
      </div>
    </Modal>
  );
...
// ─── EXTERNAL SP'S SECTION — the roster view shown in place of the in-house
// grid when spType==="external" (see the side nav in the SP's page below).
// Deliberately simpler than the in-house grid: only Active/Archive tabs (no
// Onboarding -- external SPs never go through Portal onboarding) and no
// reset-password affordance (no Portal login to reset), but otherwise
// mirrors the in-house card layout (stats row, postcode/appliance chips,
// owed banner, recent jobs) so the two rosters feel like one feature.
function ExternalSpsSection({externalSps,extTab,onExtTabChange,jobs,isOwner,accent,onAdd,onEdit,onArchive,onRestore,onDelete,onOpenJob}){
  const shown = externalSps.filter(sp=>extTab==="archived"?!!sp.archivedAt:!sp.archivedAt);
  const counts = {
    active: externalSps.filter(sp=>!sp.archivedAt).length,
    archived: externalSps.filter(sp=>!!sp.archivedAt).length,
  };
  return (
    <div>
      <div className="flex flex-col md:flex-row md:justify-between md:items-center" style={{gap:10,marginBottom:16}}>
        <h1 style={{margin:0,fontSize:19,fontWeight:900,color:C.text}}>SP's <span style={{color:C.light,fontWeight:600}}>— External SP's</span></h1>
        <Btn onClick={onAdd}>+ Add External SP</Btn>
      </div>
      <StatusTabs activeColor={accent} value={extTab} onChange={onExtTabChange} tabs={[
      ...
      {extTab==="archived" ? (
        <div style={{background:C.card,borderRadius:13,overflow:"hidden",boxShadow:"0 1px 3px rgba(0,0,0,.05)"}}>
          {shown.length===0&&<div style={{padding:22,textAlign:"center",color:C.light,fontSize:13}}>No archived external SP's.</div>}
          {shown.map((sp,i)=>(
            <div key={sp.id} style={{padding:"12px 18px",borderBottom:i<shown.length-1?`1px solid ${C.border}`:"none",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
              <Av initials={sp.name.split(" ").map(w=>w[0]).join("")||"?"} color={accent}/>
              <div style={{flex:1,minWidth:120}}>
                <div style={{fontWeight:700,fontSize:13}}>{sp.name}</div>
                <div style={{color:C.light,fontSize:11}}><CallLink phone={sp.phone}/> · {sp.email}</div>
                <div style={{color:C.light,fontSize:10,marginTop:2}}>Archived {sp.archivedAt?new Date(sp.archivedAt).toLocaleDateString("en-GB"):"—"}</div>
              </div>
              <Btn onClick={()=>onRestore(sp)} variant="ghost" sm>Restore</Btn>
              {isOwner&&<Btn onClick={()=>onDelete(sp)} variant="danger" sm>Delete Forever</Btn>}
            </div>
          ))}
        </div>
      ) : (
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
        {shown.length===0&&<div style={{color:C.light,fontSize:13}}>No external SP's yet — add one when a one-off job needs a contractor outside your normal roster.</div>}
        {shown.map(sp=>{
          const spJobs=jobs.filter(j=>j.externalEngineerId===sp.id);
          const done=spJobs.filter(j=>j.status==="Completed").length;
          const ber=spJobs.filter(j=>j.status==="Beyond Repair").length;
          const total=done+ber;
          const sr=total?Math.round(done/total*100):null;
          const active=spJobs.filter(j=>!["Completed","Beyond Repair","Cancelled"].includes(j.status)).length;
          const owed=spJobs.filter(j=>j.status==="Completed"&&!j.paid).reduce((s,j)=>s+Number(j.rate||0),0);
          return (
            <div key={sp.id} style={{background:C.card,borderRadius:13,boxShadow:"0 1px 3px rgba(0,0,0,.05)",overflow:"hidden"}}>
              <div style={{padding:"13px 15px",borderBottom:`1px solid ${C.border}`,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                <Av initials={sp.name.split(" ").map(w=>w[0]).join("")||"?"} color={accent}/>
                <div style={{flex:1,minWidth:120}}>
                  <div style={{fontWeight:800,fontSize:14}}>{sp.name}</div>
                  <div style={{color:C.light,fontSize:11}}>
                    {sp.company||sp.phone||sp.email
                      ? [sp.company,sp.phone&&<CallLink key="ph" phone={sp.phone}/>,sp.email].filter(Boolean).reduce((acc,el,i)=>i===0?[el]:[...acc," · ",el],[])
                      : "No contact details"}
                  </div>
                </div>
                {isOwner&&sp.rate!=null&&<span style={{fontWeight:800,color:accent,fontSize:13}}>£{sp.rate}/job</span>}
                <Btn onClick={()=>onEdit(sp)} variant="ghost" sm>Edit</Btn>
                <Btn onClick={()=>onArchive(sp)} variant="ghost" sm style={{color:C.danger}}>Archive</Btn>
              </div>
              ...
              <div style={{padding:"9px 15px",borderBottom:`1px solid ${C.border}`}}>
                ...
                <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                  {sp.postcodes.length?sp.postcodes.map(pc=><span key={pc} style={{background:C.purpleLight,color:C.purple,fontSize:10,fontWeight:700,padding:"2px 6px",borderRadius:4}}>{pc}</span>):<span style={{color:C.light,fontSize:11,fontStyle:"italic"}}>None set</span>}
                </div>
              </div>
              <div style={{padding:"9px 15px",borderBottom:`1px solid ${C.border}`}}>
                ...
                <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                  {sp.applianceTypes.length?sp.applianceTypes.map(a=><span key={a} style={{background:C.successLight,color:C.success,fontSize:10,fontWeight:700,padding:"2px 6px",borderRadius:4}}>{a}</span>):<span style={{color:C.light,fontSize:11,fontStyle:"italic"}}>None set</span>}
                </div>
              </div>
              ...
              <div style={{padding:"8px 15px"}}>
                {spJobs.filter(j=>!["Completed","Beyond Repair","Cancelled"].includes(j.status)).slice(0,2).map(j=>(
                  <div key={j.id} onClick={()=>onOpenJob(j)} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12,padding:"4px 0",cursor:"pointer",color:C.mid}} onMouseOver={e=>e.currentTarget.style.color=accent} onMouseOut={e=>e.currentTarget.style.color=C.mid}>
                    <span><strong style={{color:C.text}}>{j.customer}</strong> · {j.appliance}{j.brand?` · ${j.brand}`:""}</span><Badge status={j.status}/>
                  </div>
                ))}
                {!spJobs.filter(j=>!["Completed","Beyond Repair","Cancelled"].includes(j.status)).length&&<div style={{fontSize:11,color:C.light,fontStyle:"italic"}}>No active jobs</div>}
              </div>
            </div>
          );
        })}
...
function AddEngineerModal({onCreated,onCancel,defaultType}) {
  const [f,setF] = useState({name:"",email:"",phone:"",rate:45,type:defaultType||"appliance",provisioning:"invite",password:""});
  const [busy,setBusy] = useState(false);
  const [err,setErr] = useState("");
  const set=(k,v)=>setF(p=>({...p,[k]:v}));
  // Brightened field style for this modal only -- same as `inp` but with the
  // barely-visible dark-grey border swapped for the brand lime/green
  // (C.primary, same colour as the Create SP button below), per Ini's report
  // that the box and its fields were too dark to make out.
  const spInp = {...inp, border:`1.5px solid ${C.primary}`};
  ...
  const submit = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("create-user", {
      body: {
        ...
        engineer:{ payRate:Number(f.rate)||45, type:f.type, postcodes:[], applianceTypes:[], brandExclusions:{}, selfServiceEnabled:true },
      },
  ...
  return (
    <Modal title="Add SP" onClose={onCancel} accent>
      {err&&<div style={{background:"rgba(248,113,113,0.15)",color:"#f87171",borderRadius:7,padding:"8px 12px",fontSize:13,marginBottom:12,fontWeight:600}}>{err}</div>}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        <Fl label="Full Name"><input style={spInp} value={f.name} onChange={e=>set("name",e.target.value)}/></Fl>
        <Fl label="Phone"><input style={spInp} value={f.phone} onChange={e=>set("phone",e.target.value)} placeholder="07..."/></Fl>
        <Fl label="Email"><input style={spInp} value={f.email} onChange={e=>set("email",e.target.value)} placeholder="sp@easyrepair.co.uk"/></Fl>
        <Fl label="Pay Rate (£ per job)"><input type="number" style={spInp} value={f.rate} onChange={e=>set("rate",e.target.value)}/></Fl>
        <div style={{gridColumn:"1 / -1"}}>
          <Fl label="SP Type">
            <select style={spInp} value={f.type} onChange={e=>set("type",e.target.value)}>
              {SP_TYPES.map(t=><option key={t.key} value={t.key}>{t.label}</option>)}
            </select>
          </Fl>
        </div>
      ...
      <div style={{background:C.primaryLight,border:`1px solid #BFDBFE`,borderRadius:9,padding:"12px 14px",marginTop:6,marginBottom:14}}>
        ...
        {f.provisioning==="password"&&(
          <div style={{marginTop:10}}>
            <input style={spInp} type="text" value={f.password} onChange={e=>set("password",e.target.value)} placeholder="Temporary password (min 8 characters)"/>
...
// ─── ENGINEER/SP — FULL JOB LIST — shown when an SP card on the SP's page is
// clicked (see selEngJobs in App). The card itself only ever showed up to 2
// active jobs as a teaser; this is every job ever assigned to that SP,
// newest first, each clickable through to the full JobDetail modal (same
// "close this, open that" hand-off PlanDetail's onOpenJob already uses).
function EngineerJobsList({eng,jobs,onOpenJob}) {
  const eJobs = jobs.filter(j=>j.engineerId===eng.id)
    .slice().sort((a,b)=>new Date(b.createdAt||0)-new Date(a.createdAt||0));
  if(!eJobs.length) return <div style={{color:C.light,fontSize:13,fontStyle:"italic",padding:"4px 0 8px"}}>No jobs assigned to {eng.name} yet.</div>;
  return (
    <div>
      <div style={{fontSize:11,color:C.light,marginBottom:10}}>{eJobs.length} job{eJobs.length===1?"":"s"} ever assigned to {eng.name}, newest first.</div>
      <div style={{display:"flex",flexDirection:"column"}}>
        {eJobs.map((j,i)=>(
          <div key={j.id} onClick={()=>onOpenJob(j)}
               style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,padding:"10px 4px",borderTop:i>0?`1px solid ${C.border}`:"none",cursor:"pointer"}}
               onMouseOver={e=>e.currentTarget.style.background="#1E2530"} onMouseOut={e=>e.currentTarget.style.background="transparent"}>
            <div style={{minWidth:0}}>
              <div style={{fontWeight:700,fontSize:13,color:C.text,display:"flex",alignItems:"center",gap:6}}>{j.customer}{j.isTest&&<TestPill/>}</div>
              <div style={{fontSize:11,color:C.light,marginTop:1}}>{j.bookingRef||('#'+j.id)} · {j.appliance}{j.brand?` · ${j.brand}`:""}{j.scheduledDate?` · ${fmt(j.scheduledDate)}`:""}</div>
            </div>
...
function mapEngineerRow(row){
  return {
    ...
    // Defaults to "appliance" for any row from before 0005_engineer_types.sql
    // added the column (or if it's ever null for some other reason).
    type: row.type||"appliance",
    ...
    // Set by the generate-test-data Edge Function (Settings -> System ->
    // Test) -- never by anything a person fills in themselves.
    isTest: !!row.profile?.is_test,
  };
}

// Maps a Supabase `external_engineers` row (see 0007_external_sps.sql) to the
// flat shape ExternalSpEditor/JobForm/ReassignModal work with -- and the
// reverse, for writes. No profile/auth row exists for these (no Portal
// login), so this is much flatter than mapEngineerRow above: no email
// disabled-field-from-profiles dance, no working hours, no onboarding.
function mapExternalRow(row){
  return {
    id: row.id, name: row.name||"", phone: row.phone||"", email: row.email||"", company: row.company||"",
    rate: row.pay_rate, postcodes: row.postcodes||[], applianceTypes: row.appliance_types||[],
...
function externalSpContactFieldsToRow(f){
  return {
    name: f.name, phone: f.phone||null, email: f.email||null, company: f.company||null,
    pay_rate: f.rate===""||f.rate==null?null:Number(f.rate),
  };
}
function externalSpFieldsToRow(f){
  return {
    ...externalSpContactFieldsToRow(f),
    postcodes: f.postcodes||[], appliance_types: f.applianceTypes||[], brand_exclusions: f.brandExclusions||{},
    insurance_expiry_date: f.insuranceExpiryDate||null, notes: f.notes||null,
  };
}
...
function mapBookingRow(row){
  return {
    ...
    // Set only when this booking's external SP was picked from the roster
    // (see 0007_external_sps.sql) rather than typed as a genuine one-off --
    // null does not mean "not external", just "not linked to a roster row".
    externalEngineerId: row.external_engineer_id||null,
    // Did the customer choose "Repair + Protection Plan" (vs. repair only)
    // at booking time -- see 0006_service_plans.sql. Setting this true
    // (here or via the public booking form's submit-booking Edge Function)
    // is what the trg_sync_service_plan_on_booking_insert/update triggers
    // watch for to create the matching public.service_plans row that the
    // Protection Plans page reads from.
    wantsProtectionPlan: !!row.wants_protection_plan,
    // Set by the generate-test-data Edge Function (Settings -> System ->
    // Test) -- never by anything a person fills in themselves.
    isTest: !!row.is_test,
  };
...
function bookingFieldsToRow(form){
  return {
    ...
    external_engineer_id: form.isExternal?(form.externalEngineerId||null):null,
    wants_protection_plan: !!form.wantsProtectionPlan,
  };
}

// Maps a `service_plans` row (see 0006_service_plans.sql) into the shape
// the Protection Plans page works with. One row per customer who opted
// into "Repair + Protection Plan" -- created automatically by a DB trigger
// off `bookings.wants_protection_plan`, not written directly by the app
// (aside from flipping `status` when staff cancel/reactivate one below).
function mapPlanRow(row){
  return {
    id: row.id, bookingId: row.booking_id||null,
    customer: row.customer||"", phone: row.phone||"", email: row.email||"",
    address: row.address||"", postcode: row.postcode||"",
    appliance: row.appliance||"", brand: row.brand||"",
    applianceAge: row.appliance_age??"",
    source: row.source||"Website", status: row.status||"active",
    startedAt: row.started_at||null, cancelledAt: row.cancelled_at||null,
    createdAt: row.created_at||null,
    isTest: !!row.is_test,
  };
}

// Detail view for a single Protection Plans row -- shows everything staff
// need to know about the customer covered by this plan (full name, full
// address, and the appliance it covers) without having to open the
// original booking. `linkedJob` is the matching row from `jobs` (may be
// null if the booking was later deleted -- service_plans.booking_id is
// ON DELETE SET NULL, see 0006_service_plans.sql).
function PlanDetail({plan,linkedJob,onClose,onCancelPlan,onReactivatePlan,onOpenJob}) {
  const fullAddress = [plan.address, plan.postcode].filter(Boolean).join(", ");
  return (
    <div>
      <div style={{display:"flex",gap:7,marginBottom:14,flexWrap:"wrap"}}>
        <span style={{background:plan.status==="active"?C.purpleLight:"rgba(100,116,139,0.15)",color:plan.status==="active"?C.purple:"#64748B",padding:"2px 9px",borderRadius:20,fontSize:11,fontWeight:700,display:"inline-flex",alignItems:"center",gap:4}}>
          <span style={{width:6,height:6,borderRadius:"50%",background:plan.status==="active"?C.purple:"#64748B",flexShrink:0}}/>{plan.status==="active"?"Active":"Cancelled"}
        </span>
        {plan.isTest&&<TestPill/>}
      </div>

      {/* Appliance banner -- the appliance-specific fields the plan covers */}
      <div style={{background:C.purpleLight,border:"1px solid rgba(168,85,247,0.25)",borderRadius:9,padding:"10px 16px",marginBottom:14,display:"flex",gap:24,flexWrap:"wrap",alignItems:"center"}}>
        {[["Appliance",plan.appliance||"Unknown"],["Brand",plan.brand||"Unknown"],["Age",(plan.applianceAge===""||plan.applianceAge==null)?"Unknown":`${plan.applianceAge} yr${plan.applianceAge!=1?"s":""}`]].map(([l,v])=>(
          <div key={l}><div style={{fontSize:10,color:C.light,fontWeight:700,textTransform:"uppercase"}}>{l}</div><div style={{fontWeight:700,fontSize:14,color:C.text}}>{v}</div></div>
        ))}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
        <div style={{background:"#1E2530",borderRadius:9,padding:12}}>
          <div style={{fontSize:10,color:C.light,fontWeight:700,textTransform:"uppercase",marginBottom:5}}>Full Name</div>
          <div style={{fontWeight:700,fontSize:14}}>{plan.customer||"Unknown"}</div>
          <div style={{color:C.mid,fontSize:12,marginTop:6}}>📞 {plan.phone||"—"}</div>
          <div style={{color:C.mid,fontSize:12,marginTop:2}}>✉ {plan.email||"—"}</div>
        </div>
        <div style={{background:"#1E2530",borderRadius:9,padding:12}}>
          <div style={{fontSize:10,color:C.light,fontWeight:700,textTransform:"uppercase",marginBottom:5}}>Full Address</div>
          <div style={{fontSize:13}}>{plan.address||<span style={{color:C.light}}>—</span>}</div>
          {plan.postcode&&<div style={{fontSize:13,fontWeight:700,color:C.text,marginTop:3,letterSpacing:.3}}>{plan.postcode}</div>}
          {fullAddress&&<a href={`https://maps.google.com?q=${encodeURIComponent(fullAddress)}`} target="_blank" rel="noreferrer" style={{fontSize:12,color:C.primary,marginTop:3,display:"inline-block"}}>Open in Maps →</a>}
        </div>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:18}}>
        <div><div style={{fontSize:10,color:C.light,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>Plan Started</div><div style={{fontSize:13}}>{plan.startedAt?new Date(plan.startedAt).toLocaleDateString("en-GB"):"—"}</div></div>
        {plan.status==="cancelled"&&<div><div style={{fontSize:10,color:C.light,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>Cancelled</div><div style={{fontSize:13}}>{plan.cancelledAt?new Date(plan.cancelledAt).toLocaleDateString("en-GB"):"—"}</div></div>}
        <div><div style={{fontSize:10,color:C.light,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>Source</div><div style={{fontSize:13}}>{plan.source||"—"}</div></div>
        <div><div style={{fontSize:10,color:C.light,fontWeight:700,textTransform:"uppercase",marginBottom:3}}>Linked Booking</div><div style={{fontSize:13}}>{linkedJob?<button onClick={()=>onOpenJob(linkedJob)} style={{background:"none",border:"none",color:C.primary,fontWeight:800,cursor:"pointer",fontFamily:"inherit",padding:0,fontSize:13}}>{linkedJob.bookingRef||('#'+linkedJob.id)}</button>:<span style={{color:C.light}}>—</span>}</div></div>
      </div>

      <div style={{display:"flex",gap:10}}>
        {plan.status==="active"
          ? <Btn onClick={onCancelPlan} variant="ghost" full style={{padding:"10px 0"}}>Cancel Plan</Btn>
          : <Btn onClick={onReactivatePlan} variant="ghost" full style={{padding:"10px 0"}}>Reactivate Plan</Btn>}
        <Btn onClick={onClose} variant="ghost" full style={{padding:"10px 0"}}>Close</Btn>
      </div>
    </div>
  );
}
...
// Builds and downloads a CSV of the given service_plans rows (whatever's
// currently on screen on the Protection Plans page -- respects the
// Active/Cancelled tab and search box, same as the table itself). `jobs`
// is only needed to resolve each plan's friendly booking reference.
function exportPlansCSV(rows, jobs, statusLabel){
  const headers = ["Full Name","Phone","Email","Address","Postcode","Appliance","Brand","Appliance Age (yrs)","Status","Plan Started","Cancelled Date","Source","Linked Booking","Test Data"];
  const lines = [headers.map(csvField).join(",")];
  rows.forEach(p=>{
    const linkedJob = jobs.find(j=>j.id===p.bookingId);
    lines.push([
      p.customer, p.phone, p.email, p.address, p.postcode,
      p.appliance, p.brand,
      (p.applianceAge===""||p.applianceAge==null)?"":p.applianceAge,
      p.status==="active"?"Active":"Cancelled",
      p.startedAt?new Date(p.startedAt).toLocaleDateString("en-GB"):"",
      p.cancelledAt?new Date(p.cancelledAt).toLocaleDateString("en-GB"):"",
      p.source,
      linkedJob?(linkedJob.bookingRef||('#'+linkedJob.id)):"",
      p.isTest?"Yes":"No",
    ].map(csvField).join(","));
  });
  // Leading BOM so Excel (still the most common opener for a CSV like this)
  // detects UTF-8 instead of mangling the £/em-dash characters address text
  // sometimes carries.
  const blob = new Blob(["﻿"+lines.join("\r\n")], {type:"text/csv;charset=utf-8;"});
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = `protection-plans-${statusLabel}-${new Date().toISOString().slice(0,10)}.csv`;
...
// Settings -> System's own sub-nav, same shape as Website's. Just "Test"
// for now (System -> Test -> Test Jobs/Test SP's), but laid out the same
// way so another System area can be added the same way Website's were.
const SYSTEM_SUBTABS = [
  {key:"test",label:"Test"},
];

// ─── SYSTEM -> TEST -- system-generated Test Jobs / Test SP's ────────────
// Settings for the two generators live in the `test_generators` table (one
// row each, id 'jobs'/'sps') and are actually acted on by the
// generate-test-data Edge Function -- either on the schedule set up via
// pg_cron (see 0005_test_generators.sql), or immediately via the
// "Generate one batch now" button below, which just calls the same
// function with { force:true, only:<type> }.
//
// Volume is a 0-100 "how often this actually fires" dial rather than a
// count -- see the comment on test_generators.volume in the migration for
// why. Amount per run is the literal number of test rows created each time
// it does fire.
const TEST_GENERATOR_TYPES = [
  {key:"jobs",label:"Test Jobs",noun:"test jobs",
    blurb:"Drops synthetic bookings into the real Bookings list (customer, appliance, issue -- all clearly fake) so you can see the dashboard, notifications and lists behave under real volume. Always created unassigned, never dispatched to a real SP."},
  {key:"sps",label:"Test SP's",noun:"test SP's",
    blurb:"Creates real (but inert) Service Provider accounts so the Engineers screen can be tested at volume. Created inactive with no postcodes or skills, so the real auto-assign logic can never actually hand one a genuine customer's job."},
];
const TEST_INTERVAL_OPTIONS = [
  {key:"15m",label:"Every 15 minutes"},
  {key:"30m",label:"Every 30 minutes"},
  {key:"45m",label:"Every 45 minutes"},
  {key:"1h", label:"Every hour"},
  {key:"2h", label:"Every 2 hours"},
  {key:"3h", label:"Every 3 hours"},
  {key:"1d", label:"Once a day"},
];
...
function TestGeneratorsPanel({onReloadJobs,onReloadEngineers}){
  const [genKey,setGenKey] = useState("jobs");
  const [configs,setConfigs] = useState(null);   // {jobs:row, sps:row}
  const [loadErr,setLoadErr] = useState("");
  const [draft,setDraft] = useState(null);       // editable copy of configs[genKey]
  const [saving,setSaving] = useState(false);
  const [saveErr,setSaveErr] = useState("");
  const [saveOk,setSaveOk] = useState(false);
  const [genBusy,setGenBusy] = useState(false);
  const [genErr,setGenErr] = useState("");
  const [clearTarget,setClearTarget] = useState(null); // {key,label}
  const [clearBusy,setClearBusy] = useState(false);
  const [clearErr,setClearErr] = useState("");

  const loadConfigs = async () => {
    const { data, error } = await supabase.from("test_generators").select("*");
    if(error){ setLoadErr(`Couldn't load test generator settings: ${error.message}`); return; }
    setLoadErr("");
    setConfigs(Object.fromEntries((data||[]).map(r=>[r.id,r])));
  };
  useEffect(()=>{ loadConfigs(); },[]);
  useEffect(()=>{
    if(configs?.[genKey]) setDraft({...configs[genKey]});
    setSaveErr(""); setSaveOk(false); setGenErr("");
  },[genKey,configs]);

  const active = TEST_GENERATOR_TYPES.find(t=>t.key===genKey);
  const setD = (k,v) => setDraft(p=>({...p,[k]:v}));

  const save = async () => {
    setSaving(true); setSaveErr(""); setSaveOk(false);
    const { error } = await supabase.from("test_generators").update({
      enabled: !!draft.enabled,
      volume: Number(draft.volume),
      amount_per_run: Number(draft.amount_per_run),
      interval_key: draft.interval_key,
    }).eq("id",genKey);
    setSaving(false);
  ...
  const generateNow = async () => {
    ...
    const { data, error } = await supabase.functions.invoke("generate-test-data", {
      body: { force:true, only:genKey },
      headers: { Authorization:`Bearer ${sess?.session?.access_token}` },
    });
    setGenBusy(false);
    if(error||data?.error){ setGenErr(await edgeFnErrorMessage(error, data?.error||"Something went wrong generating test data.")); return; }
    await loadConfigs();
    if(genKey==="jobs") await onReloadJobs?.(); else await onReloadEngineers?.();
  };

  const runClear = async () => {
    if(!clearTarget) return;
    setClearBusy(true); setClearErr("");
    const { data: sess } = await supabase.auth.getSession();
    const { data, error } = await supabase.functions.invoke("clear-test-data", {
      body: { type: clearTarget.key },
      headers: { Authorization:`Bearer ${sess?.session?.access_token}` },
    });
    setClearBusy(false);
    if(error||data?.error){ setClearErr(await edgeFnErrorMessage(error, data?.error||"Something went wrong clearing test data.")); return; }
    setClearTarget(null);
    if(clearTarget.key==="jobs") await onReloadJobs?.(); else await onReloadEngineers?.();
  };

  if(loadErr) return <div style={{color:C.danger,fontSize:13}}>{loadErr}</div>;
  if(!configs||!draft) return <div style={{color:C.mid,fontSize:13}}>Loading…</div>;

  return (
    <div style={{display:"flex",gap:20,flexWrap:"wrap"}}>
      {/* Side menu -- Test Jobs / Test SP's */}
      <div style={{width:170,flexShrink:0,display:"flex",flexDirection:"column",gap:4}}>
        {TEST_GENERATOR_TYPES.map(t=>(
          <button key={t.key} onClick={()=>setGenKey(t.key)} style={{
            textAlign:"left",border:"none",cursor:"pointer",fontFamily:"inherit",
            padding:"9px 12px",borderRadius:8,fontSize:13,fontWeight:800,
            background:genKey===t.key?C.primaryLight:"transparent",
            color:genKey===t.key?C.primary:C.mid,
            display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,
          }}>
            {t.label}
            {configs[t.key]?.enabled&&<span style={{width:7,height:7,borderRadius:"50%",background:C.success,flexShrink:0}} title="Enabled"/>}
          </button>
        ))}
      </div>

      {/* Config panel */}
      <div style={{flex:1,minWidth:280,background:C.card,border:`1px solid ${C.border}`,borderRadius:13,padding:18}}>
        <div style={{fontSize:14,fontWeight:800,color:C.text,marginBottom:4}}>{active.label}</div>
        <div style={{fontSize:12,color:C.mid,lineHeight:1.5,marginBottom:16}}>{active.blurb}</div>

        <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",marginBottom:18,fontSize:13,fontWeight:700}}>
          <input type="checkbox" checked={!!draft.enabled} onChange={e=>setD("enabled",e.target.checked)}/>
          Enabled (runs automatically on schedule below)
        </label>

        <Fl label={`Volume — ${draft.volume}% chance it fires each time it's due`}>
          <input type="range" min={0} max={100} step={5} value={draft.volume} onChange={e=>setD("volume",Number(e.target.value))} style={{width:"100%",accentColor:C.primary}}/>
          <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:C.light,marginTop:2}}>
            <span>Rare</span><span>Always</span>
          </div>
        </Fl>

        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <Fl label="Amount at a time">
            <input type="number" min={1} max={50} style={inp} value={draft.amount_per_run} onChange={e=>setD("amount_per_run",e.target.value)}/>
          </Fl>
          <Fl label="Interval">
            <select style={inp} value={draft.interval_key} onChange={e=>setD("interval_key",e.target.value)}>
              {TEST_INTERVAL_OPTIONS.map(o=><option key={o.key} value={o.key}>{o.label}</option>)}
            </select>
          </Fl>
        </div>

        {saveErr&&<div style={{background:"rgba(248,113,113,0.15)",color:C.danger,borderRadius:7,padding:"8px 12px",fontSize:12,marginBottom:12,fontWeight:600}}>{saveErr}</div>}
        <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
          <Btn onClick={save} sm style={{opacity:saving?.7:1}}>{saving?"Saving…":"Save"}</Btn>
          {saveOk&&<span style={{color:C.success,fontSize:12,fontWeight:700,alignSelf:"center"}}>Saved ✓</span>}
        </div>

        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:14,marginBottom:14}}>
          <div style={{fontSize:11,color:C.mid,marginBottom:8}}>
            Last run: <strong style={{color:C.text}}>{relTime(configs[genKey]?.last_run_at)}</strong>
            {configs[genKey]?.last_run_status&&<> · created <strong style={{color:C.text}}>{configs[genKey]?.last_run_created??0}</strong> · <span style={{color:/error/.test(configs[genKey].last_run_status)?C.danger:C.mid}}>{configs[genKey].last_run_status}</span></>}
          </div>
          {genErr&&<div style={{background:"rgba(248,113,113,0.15)",color:C.danger,borderRadius:7,padding:"8px 12px",fontSize:12,marginBottom:10,fontWeight:600}}>{genErr}</div>}
          <Btn onClick={generateNow} variant="ghost" sm style={{opacity:genBusy?.7:1}}>{genBusy?"Generating…":`Generate one batch of ${active.noun} now`}</Btn>
        </div>

        <div style={{borderTop:`1px solid ${C.border}`,paddingTop:14}}>
          <div style={{fontSize:10,fontWeight:700,color:C.light,textTransform:"uppercase",letterSpacing:.6,marginBottom:8}}>Danger Zone</div>
          <Btn onClick={()=>{setClearErr("");setClearTarget({key:genKey,label:active.noun});}} variant="danger" sm>Clear all {active.noun}</Btn>
        </div>
      </div>

      {clearTarget&&(
        <ConfirmModal
          title={`Clear all ${clearTarget.label}?`}
          message={`This permanently deletes every ${clearTarget.key==="jobs"?"test booking":"test SP account"} this generator has created. Real bookings and real SPs are never touched. This can't be undone.`}
          confirmLabel="Clear test data" danger busy={clearBusy} err={clearErr}
          onConfirm={runClear} onCancel={()=>setClearTarget(null)}
        />
      )}
    </div>
  );
...
function SettingsView({pricing,onReload,users,onUserCreated,currentUserId,usersTab,onUsersTabChange,onArchive,onRestore,onDelete,onResetPassword,onReloadJobs,onReloadEngineers}){
  const [editArea,setEditArea] = useState(null);
  const [search,setSearch] = useState("");
  const [section,setSection] = useState("website"); // "system" | "website" | "users" | "payments" | "assignment"
  const [websiteTab,setWebsiteTab] = useState("pricing"); // subsection of Website
  const [systemTab,setSystemTab] = useState("test"); // subsection of System
  ...
  return (
    <div>
      ...
      {section==="system" && (
        <div>
          <SettingsSubTabs tabs={SYSTEM_SUBTABS} value={systemTab} onChange={setSystemTab}/>
          {systemTab==="test" && <TestGeneratorsPanel onReloadJobs={onReloadJobs} onReloadEngineers={onReloadEngineers}/>}
        </div>
      )}
...
export default function App() {
  ...
  const [fsPreset,setFsPreset]    = useState("");           // "" | "today" | "unassigned" | "active" -- set by clicking a Dashboard stat card; shown as a removable chip on Bookings
  const [qSearch,setQSearch]      = useState("");           // search box on the Quotes page
  const [globalSearch,setGlobalSearch]       = useState(""); // sidebar "jump to booking" box -- keyed on booking_ref
  ...
  const [spView,setSpView]        = useState("card");     // "card" | "list" -- view toggle for the in-house SP's grid (Active/Onboarding tabs only; Archive is already a compact list)
  const [spType,setSpType]        = useState("appliance"); // which SP type the Engineers page side nav has selected -- "external" is a special value for the External SP's roster, not a real row in SP_TYPES
  const [externalSps,setExternalSps]   = useState([]);     // External SP's roster (0007_external_sps.sql)
  const [showAddExternalSp,setShowAddExternalSp] = useState(false);
  const [editExternalSp,setEditExternalSp] = useState(null);
  const [extTab,setExtTab]        = useState("active");    // "active" | "archived" -- External SP's page's own StatusTabs (no "onboarding": no Portal account to onboard)
  const [extConfirm,setExtConfirm]     = useState(null);   // {kind:"archive"|"delete", id, label}
  const [extConfirmBusy,setExtConfirmBusy] = useState(false);
  const [extConfirmErr,setExtConfirmErr]   = useState("");
  ...
  const [plans,setPlans]             = useState([]);          // Protection Plans page -- public.service_plans rows
  const [planSearch,setPlanSearch]   = useState("");
  const [planStatusTab,setPlanStatusTab] = useState("active"); // "active" | "cancelled"
  const [planStartedFrom,setPlanStartedFrom] = useState("");   // Protection Plans "Plan Started" date filter -- inclusive, yyyy-mm-dd, "" = no bound
  const [planStartedTo,setPlanStartedTo]     = useState("");   // inclusive, yyyy-mm-dd, "" = no bound
  const [selPlan,setSelPlan]         = useState(null);         // plan row shown in the detail modal, or null when closed
  const [selEngJobs,setSelEngJobs]   = useState(null);         // engineer/SP whose full job list is shown in the detail modal, or null when closed
  ...
  const loadEngineers = async () => {
    const { data, error } = await supabase.from("engineers").select("*, profile:profiles(name,phone,email,archived_at,must_change_password,terms_accepted_at,is_test)").order("created_at");
  ...
  // Protection Plans -- one row per customer who opted into "Repair +
  // Protection Plan" at booking time. Rows are created automatically by the
  // trg_sync_service_plan_on_booking_insert/update triggers on `bookings`
  // (see 0006_service_plans.sql) whenever wants_protection_plan is true, so
  // this table is never written to directly except to flip `status` when
  // staff cancel/reactivate a plan below.
  const loadPlans = async () => {
    const { data, error } = await supabase.from("service_plans").select("*").order("created_at",{ascending:false});
    if(error){ setLoadError(`Couldn't load protection plans: ${error.message}`); return; }
    setLoadError(""); setPlans((data||[]).map(mapPlanRow));
  };
  const setPlanStatus = async (id,status) => {
    await supabase.from("service_plans").update({status,cancelled_at:status==="cancelled"?new Date().toISOString():null}).eq("id",id);
    await loadPlans();
  };
  ...
  // External SP's roster (see 0007_external_sps.sql) -- separate from
  // `engineers` entirely, since there's no profiles/auth row backing these.
  const loadExternalSps = async () => {
    const { data, error } = await supabase.from("external_engineers").select("*").order("created_at");
    if(error){ setLoadError(`Couldn't load external SP's: ${error.message}`); return; }
    setLoadError(""); setExternalSps((data||[]).map(mapExternalRow));
  };
  const saveExternalSp = async (spData) => {
    await supabase.from("external_engineers").update(externalSpFieldsToRow(spData)).eq("id",spData.id);
    await loadExternalSps();
    setEditExternalSp(null);
  };
  // Archive/restore/delete are all plain table operations here, unlike the
  // profiles-backed equivalents above -- there's no auth.users row, invite,
  // or Portal session tied to an external SP, so no Edge Function (and no
  // service_role key) is needed for any of these, including permanent delete.
  const archiveExternalSp = async (id) => { await supabase.from("external_engineers").update({archived_at:new Date().toISOString()}).eq("id",id); await loadExternalSps(); };
  const restoreExternalSp = async (id) => { await supabase.from("external_engineers").update({archived_at:null}).eq("id",id); await loadExternalSps(); };
  const askArchiveExt = (id,label) => { setExtConfirmErr(""); setExtConfirm({kind:"archive",id,label}); };
  const askDeleteExt  = (id,label) => { setExtConfirmErr(""); setExtConfirm({kind:"delete",id,label}); };
  const runExtConfirm = async () => {
    if(!extConfirm) return;
    setExtConfirmBusy(true); setExtConfirmErr("");
    try{
      if(extConfirm.kind==="archive") await archiveExternalSp(extConfirm.id);
      else { const { error } = await supabase.from("external_engineers").delete().eq("id",extConfirm.id); if(error) throw new Error(error.message); await loadExternalSps(); }
      setExtConfirm(null);
    }catch(e){ setExtConfirmErr(e.message||"Something went wrong."); }
    setExtConfirmBusy(false);
  };
  ...
  useEffect(()=>{ if(currentUser){ loadEngineers(); loadStaffUsers(); loadBookings(); loadPlans(); loadExternalSps(); if(currentUser.role==="owner") loadPricing(); } },[currentUser]);
  ...
  // Live sync of `bookings` -- keeps the `jobs` state (and therefore the
  // dashboard, Today's Bookings, Today's Schedule, the board, etc.) current
  // without a page reload whenever a row changes, from ANY source: staff
  // editing here, the public website's submit-booking Edge Function, or --
  // the specific case that was broken -- an engineer marking a job
  // "Completed" from Portal. Previously this subscription only listened for
  // INSERT and only used it to fire a desktop notification; it never wrote
  // the change into `jobs`, so nothing on this dashboard ever updated live,
  // not even for new bookings (only the notification made it look otherwise).
  //
  // Merges every event straight into `jobs` state via mapBookingRow, same
  // mapping loadBookings() uses, so this can't drift out of sync with what a
  // full reload would show:
  //   - INSERT: prepend the new row (loadBookings sorts created_at desc, so
  //     a brand-new booking belongs at the front) unless it's already there.
  //   - UPDATE: replace the matching row in place (this is what makes a
  //     Portal "Completed" status change, or a reassignment, appear here
  //     immediately).
  //   - DELETE: drop the row.
  // The desktop notification behaviour on INSERT is unchanged.
  //
  ...
  useEffect(()=>{
    ...
    const channel = supabase
      .channel(`bookings-sync-${currentUser.id}`)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"bookings"},(payload)=>{
        const row = mapBookingRow(payload.new||{});
  ...
  // Archived external SP's drop out of the assignment pickers the same way
  // archived/onboarding in-house engineers do, but stay resolvable elsewhere
  // (Payments, job history) via the full `externalSps` list.
  const activeExternalSps = externalSps.filter(s=>!s.archivedAt);
  ...
  // tab.
  // Dashboard stat-card shortcuts -- open Bookings with just that preset
  // applied (other filters reset so the list matches the card's count).
  const openBookingsPreset = preset => {
    setFsStatus("All"); setFsEng("All"); setFsSearch(""); setFsPreset(preset); setView("jobs");
  };
  const FS_PRESET_LABELS = {today:"Today's bookings",unassigned:"Unassigned (need SP)",active:"Active bookings"};
  // Drop the preset once the user navigates away from Bookings, so opening
  // Bookings normally from the sidebar later shows the full list again.
  useEffect(()=>{ if(view!=="jobs") setFsPreset(""); },[view]);

  const filtJobs = jobs.filter(j=>{
    if(j.status==="Quote")return false;
    if(fsPreset==="today"&&!(j.createdAt&&localDateStr(new Date(j.createdAt))===TODAY))return false;
    if(fsPreset==="unassigned"&&(j.engineerId||j.isExternal||["Cancelled","Completed","Beyond Repair"].includes(j.status)))return false;
    if(fsPreset==="active"&&["Cancelled","Completed","Beyond Repair"].includes(j.status))return false;
  ...
  // Protection Plans page -- filtered by the Active/Cancelled pill plus a
  // free-text search across the same kind of fields the Bookings search
  // covers.
  const filtPlans = plans.filter(p=>{
    if(p.status!==planStatusTab) return false;
    if(planSearch && ![p.customer,p.phone,p.email,p.postcode,p.appliance,p.brand||""].some(s=>(s||"").toLowerCase().includes(planSearch.toLowerCase()))) return false;
    // Plan Started date filter -- p.startedAt is a full timestamptz, so it's
    // converted to the browser's local calendar day (same approach as TODAY
    // above) before comparing against the plain yyyy-mm-dd bounds from the
    // date pickers, otherwise a plan started late evening/early morning could
    // land on the wrong side of a "from"/"to" boundary for the user's timezone.
    if(planStartedFrom || planStartedTo){
      if(!p.startedAt) return false;
      const startedDay = localDateStr(new Date(p.startedAt));
      if(planStartedFrom && startedDay<planStartedFrom) return false;
      if(planStartedTo && startedDay>planStartedTo) return false;
    }
    return true;
  });

  // "Today's Schedule" below is about appointments happening today, so it
  // stays keyed on scheduledDate. The "Today's Bookings" StatCard is a
  // different question -- "how many bookings came in today" -- which is
  // createdAt, not scheduledDate (a booking made today might be scheduled
  // for next week, and a booking scheduled for today might've come in days
  // ago). Keeping these as two separate values instead of reusing one for
  // both is what was silently wrong before.
  const todayJobs  = jobs.filter(j=>j.scheduledDate===TODAY);
  // Quotes are excluded -- they only count on the Quotes page, not as bookings.
  const todayNewBookings = jobs.filter(j=>j.status!=="Quote" && j.createdAt && localDateStr(new Date(j.createdAt))===TODAY);
  ...
  // External SP's picked from the roster (0007_external_sps.sql) group by
  // their stable externalEngineerId; a genuine one-off (never linked to a
  // roster row) has no id to group by, so those fall back to name+phone --
  // same as how staff would recognise "that's the same contractor" by eye.
  const externalGroups = useMemo(()=>{
    const groups = {};
    jobs.filter(j=>j.isExternal).forEach(j=>{
      const key = j.externalEngineerId || `${(j.externalName||"").trim().toLowerCase()}|${(j.externalPhone||"").trim()}`;
      groups[key] ||= { id:j.externalEngineerId||null, name:j.externalName||"Unnamed", phone:j.externalPhone||"", email:j.externalEmail||"", company:j.externalCompany||"", jobs:[] };
      groups[key].jobs.push(j);
    });
    return Object.values(groups);
  },[jobs]);

  // Each engineer's repaired/BER stats (used by auto-assign's success-rate
  // scoring, and shown in the Portal's Dashboard) used to be kept in sync
  // right here -- staff/owner marking a job Completed/Beyond Repair via
  // "Edit This Job" would bump engineers.stats_completed/stats_ber by hand.
  // That only ever covered a status change made *through FixFlow*, though.
  // Engineers themselves marking a job Completed/Beyond Repair from the
  // Portal (the normal path now -- see the old comment this replaced) went
  // straight to `bookings` and never touched this code at all, so an
  // engineer's stats only reflected jobs staff happened to have edited by
  // hand -- which is why Portal's Success rate / Repaired-BER widgets read
  // as stuck at 0 for engineers who complete their own jobs the normal way.
  // A DB trigger on `bookings` (bookings_sync_engineer_stats) now does this
  // instead, since it fires for a status change from either app -- FixFlow's
  // own update below included. Only the local `engineers` cache refresh
  // stays here, so the change shows up here without a page reload.
  const saveJob=async (form)=>{
    if(editJob){
      ...
      const justFinished = ["Completed","Beyond Repair"].includes(form.status) && form.status!==editJob.status;
      if(justFinished && form.engineerId) await loadEngineers();
      // trg_sync_external_sp_stats (0007_external_sps.sql) does the actual
      // recompute in the DB the moment this update lands -- this just
      // refreshes the local cache so it shows up here without a reload.
      if(justFinished && form.isExternal && form.externalEngineerId) await loadExternalSps();
    } else {
  ...
  const doReassign=async (choice)=>{
    if(choice?.type==="external"){
      ...
      await supabase.from("bookings").update({
        ...
        external_engineer_id:ext.id||null,
  ...
  const saveEng=async (engData)=>{
    ...
    await supabase.from("engineers").update({
      pay_rate:engData.rate, type:engData.type||"appliance", postcodes:engData.postcodes, appliance_types:engData.applianceTypes,
  ...
  // Sidebar "jump to booking" search -- unlike the Bookings/Quotes tab search
  // boxes (which only filter *within* whichever tab/status/SP filters are
  // already set), this looks across every job regardless of status or
  // Booking-vs-Quote, and opens the match directly. Tries an exact
  // booking_ref match first (case-insensitive), then falls back to the raw
  // id/UUID (for the handful of older rows with no ref), then a partial ref
  // match so typing just "10001" still finds "ER-10001".
  const runGlobalSearch = () => {
    const q = globalSearch.trim();
    if(!q) return;
    const qLower = q.toLowerCase();
    const match = jobs.find(j=>(j.bookingRef||"").toLowerCase()===qLower)
      || jobs.find(j=>String(j.id).toLowerCase()===qLower)
      || jobs.find(j=>(j.bookingRef||"").toLowerCase().includes(qLower));
    if(match){
      setGlobalSearchErr("");
      setGlobalSearch("");
      setSelJob(match);
    } else {
      setGlobalSearchErr(`No booking found for "${q}"`);
    }
  };
  ...
  const NAV = [{id:"dashboard",label:"Dashboard",Ic:IconLayout},{id:"jobs",label:"Bookings",Ic:IconMenuLines},{id:"serviceplans",label:"Protection Plans",Ic:IconShield},{id:"engineers",label:"SP's",Ic:IconList},...(isOwner?[{id:"settings",label:"Settings",Ic:IconGear}]:[])];
  ...
  return (
    <div style={{fontFamily:"'Inter','Segoe UI',sans-serif",background:"#000000",color:C.text,minHeight:"100vh",display:"flex"}}>
      ...
      <div className="hidden md:flex" style={{width:208,background:C.sidebar,flexDirection:"column",height:"100vh",position:"sticky",top:0,flexShrink:0}}>
        ...
        <div style={{padding:"12px 13px",borderBottom:"1px solid #141414"}}>
          <input
            style={{width:"100%",boxSizing:"border-box",background:"#0D1117",border:"1px solid #262626",borderRadius:7,padding:"7px 10px",fontSize:12,color:C.text,outline:"none",fontFamily:"inherit"}}
            placeholder="Jump to booking ref…"
            value={globalSearch}
            onChange={e=>{setGlobalSearch(e.target.value);setGlobalSearchErr("");}}
            onKeyDown={e=>{if(e.key==="Enter")runGlobalSearch();}}
            aria-label="Search bookings by reference"
          />
          {globalSearchErr&&<div style={{color:C.danger,fontSize:11,marginTop:5}}>{globalSearchErr}</div>}
        </div>
      ...
      <div className="pb-[88px] md:pb-[22px]" style={{flex:1,overflowY:"auto",paddingTop:22,paddingLeft:24,paddingRight:24}}>
        <div style={{maxWidth:1080}}>
          ...
          {/* Mobile-only counterpart to the sidebar's "jump to booking ref"
              box above -- the sidebar itself is desktop/tablet-only
              ("hidden md:flex"), so phones need their own way to reach it. */}
          <div className="md:hidden" style={{marginBottom:16}}>
            <input
              style={{width:"100%",boxSizing:"border-box",background:C.card,border:`1px solid ${C.border}`,borderRadius:7,padding:"9px 12px",fontSize:13,color:C.text,outline:"none",fontFamily:"inherit"}}
              placeholder="Jump to booking ref…"
              value={globalSearch}
              onChange={e=>{setGlobalSearch(e.target.value);setGlobalSearchErr("");}}
              onKeyDown={e=>{if(e.key==="Enter")runGlobalSearch();}}
          ...
          {view==="dashboard"&&(
            <div>
              ...
              <div style={{display:"flex",gap:12,marginBottom:18,flexWrap:"wrap"}}>
                <StatCard label="Today's Bookings" value={todayNewBookings.length} color={C.primary} sub={`${todayNewBookings.filter(j=>j.status==="Completed").length} done`} onClick={()=>openBookingsPreset("today")} title="View bookings that came in today"/>
                <StatCard label="Unassigned" value={unassigned.length} color={C.warn} sub="Need SP" onClick={()=>openBookingsPreset("unassigned")} title="View bookings with no SP assigned"/>
                <StatCard label="Active Bookings" value={jobs.filter(j=>!["Cancelled","Completed","Beyond Repair","Quote"].includes(j.status)).length} color={C.blue} onClick={()=>openBookingsPreset("active")} title="View all active bookings"/>
                <StatCard label="Open Quotes" value={quoteJobs.length} color={C.purple} onClick={()=>{setQSearch("");setView("quotes");}} title="Go to Quotes"/>
                {isOwner&&<StatCard label="Outstanding Pay" value={`£${unpaidAmt}`} color={C.danger} sub={`${unpaidDone.length} unpaid`} onClick={()=>setView("payments")} title="Go to Payments"/>}
              </div>
              <div style={{background:C.card,borderRadius:12,boxShadow:"0 1px 3px rgba(0,0,0,.05)",overflow:"hidden",marginBottom:14}}>
              ...
              <WorkloadWidget tabs={[
                {
                  key:"unassigned",
                  label:"Awaiting Engineer Assignment",
                  items: unassigned.map(j=>({
                    key:j.id,
                    title:<>{j.customer} <span style={{color:C.light,fontWeight:400}}>· {j.appliance} · {j.brand||"?"}</span></>,
                    sub: fmt(j.scheduledDate),
                    right: <PBadge p={j.priority}/>,
                    onClick: ()=>setSelJob(j),
                  })),
                },
                ...(isOwner?[{
                  ...
                  items: unpaidDone.map(j=>{
                    const eng=engineers.find(e=>e.id===j.engineerId);
                    return {
                      key:j.id,
                      title:<>{j.customer} <span style={{color:C.light,fontWeight:400}}>· {j.appliance} · {j.brand||"?"}</span></>,
                      sub: j.isExternal?`${j.externalName} (External)`:eng?.name||"Unassigned",
                      right: <span style={{fontWeight:800,color:C.danger,fontSize:12}}>£{j.rate||0}</span>,
                      onClick: ()=>setSelJob(j),
                    };
                  }),
                }]:[]),
                {
                  key:"onboarding",
                  label:"Onboarding",
                  items: engineers.filter(e=>engineerStatus(e)==="onboarding").map(eng=>{
                    const prog=engineerOnboardingProgress(eng);
                    return {
                      key:eng.id,
                      title:eng.name,
                      sub: `Step ${prog.completed} of ${prog.total}${prog.next?` · Next: ${prog.next.label}`:""}`,
                      right: <span style={{fontWeight:800,color:C.warn,fontSize:12}}>{prog.total?Math.round(prog.completed/prog.total*100):0}%</span>,
                      onClick: ()=>setEditEng(eng),
                    };
                  }),
                },
              ]}/>
          ...
          {view==="jobs"&&(
            <div>
              ...
              <div style={{display:"flex",gap:9,marginBottom:12,flexWrap:"wrap"}}>
                <input style={{...inp,maxWidth:210}} placeholder="Name / address / brand / ref / #ID" value={fsSearch} onChange={e=>setFsSearch(e.target.value)}/>
                ...
                {fsPreset&&(
                  <div style={{display:"flex",alignItems:"center",gap:7,background:"rgba(212,255,60,0.08)",border:`1px solid ${C.primary}`,color:C.primary,borderRadius:20,padding:"5px 8px 5px 12px",fontSize:12,fontWeight:700}}>
                    Showing: {FS_PRESET_LABELS[fsPreset]} ({filtJobs.length})
                    <button onClick={()=>setFsPreset("")} title="Clear filter" aria-label="Clear filter" style={{background:"none",border:"none",color:C.primary,cursor:"pointer",fontSize:14,lineHeight:1,padding:"0 2px",fontFamily:"inherit"}}>✕</button>
                  </div>
                )}
              </div>
          ...
          {view==="quotes"&&(
            <div>
              ...
              <div style={{display:"flex",gap:9,marginBottom:12,flexWrap:"wrap"}}>
                <input style={{...inp,maxWidth:210}} placeholder="Name / address / brand / ref / #ID" value={qSearch} onChange={e=>setQSearch(e.target.value)}/>
              </div>
              <div style={{background:C.card,borderRadius:12,boxShadow:"0 1px 3px rgba(0,0,0,.05)",overflowX:"auto"}}>
          ...
          {/* ── PROTECTION PLANS ── */}
          {/* Every customer who opted into "Repair + Protection Plan" (vs.
              repair only) when booking, so staff can see at a glance who's
              covered for future callouts on that appliance and manage
              cancellations. Rows come from public.service_plans, created
              automatically off bookings.wants_protection_plan -- see
              mapPlanRow / loadPlans above and 0006_service_plans.sql. */}
          {view==="serviceplans"&&(
            <div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:15,flexWrap:"wrap",gap:8}}>
                <h1 style={{margin:0,fontSize:19,fontWeight:900,color:C.text}}>Protection Plans</h1>
                <Btn onClick={()=>exportPlansCSV(filtPlans,jobs,planStatusTab)} variant="ghost" sm>⬇ Export CSV{filtPlans.length?` (${filtPlans.length})`:""}</Btn>
              </div>
              <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap"}}>
                <StatCard label="Active Plans" value={plans.filter(p=>p.status==="active").length} color={C.purple}/>
                <StatCard label="Cancelled" value={plans.filter(p=>p.status==="cancelled").length} color={C.light}/>
              </div>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
                <StatusTabs activeColor={C.purple} value={planStatusTab} onChange={setPlanStatusTab} tabs={[
                  {key:"active",label:"Active",count:plans.filter(p=>p.status==="active").length},
                  {key:"cancelled",label:"Cancelled",count:plans.filter(p=>p.status==="cancelled").length},
                ]}/>
                <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
                  <div style={{display:"flex",alignItems:"center",gap:5}}>
                    <span style={{fontSize:11,color:C.light,fontWeight:700,textTransform:"uppercase",whiteSpace:"nowrap"}}>Started</span>
                    <input type="date" style={{...inp,maxWidth:140}} value={planStartedFrom} max={planStartedTo||undefined} onChange={e=>setPlanStartedFrom(e.target.value)}/>
                    <span style={{fontSize:11,color:C.light}}>to</span>
                    <input type="date" style={{...inp,maxWidth:140}} value={planStartedTo} min={planStartedFrom||undefined} onChange={e=>setPlanStartedTo(e.target.value)}/>
                  </div>
                  {(planStartedFrom||planStartedTo)&&<Btn onClick={()=>{setPlanStartedFrom("");setPlanStartedTo("");}} variant="ghost" sm>Clear dates</Btn>}
                  <input style={{...inp,maxWidth:230}} placeholder="Name / phone / email / postcode" value={planSearch} onChange={e=>setPlanSearch(e.target.value)}/>
                </div>
              </div>
              <div style={{background:C.card,borderRadius:12,boxShadow:"0 1px 3px rgba(0,0,0,.05)",overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",minWidth:820}}>
                  <thead><tr style={{background:"#161B22"}}>
                    {["Customer","Contact","Appliance","Brand","Plan Started","Linked Booking","Status",""].map(h=>(
                      <th key={h} style={{padding:"9px 13px",textAlign:"left",fontSize:10,fontWeight:700,color:C.light,textTransform:"uppercase",letterSpacing:.5,borderBottom:`1px solid ${C.border}`}}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {filtPlans.length===0?<tr><td colSpan={8} style={{padding:28,textAlign:"center",color:C.light}}>No {planStatusTab==="active"?"active":"cancelled"} protection plans{planSearch?" match that search":""}{(planStartedFrom||planStartedTo)?" in that date range":""}.</td></tr>:filtPlans.map(p=>{
                      const linkedJob = jobs.find(j=>j.id===p.bookingId);
                      return <tr key={p.id} onClick={()=>setSelPlan(p)} style={{borderBottom:"1px solid #262626",cursor:"pointer"}} onMouseOver={e=>e.currentTarget.style.background="#1E2530"} onMouseOut={e=>e.currentTarget.style.background="transparent"}>
                        <td style={{padding:"9px 13px"}}><div style={{fontWeight:700,fontSize:13,display:"flex",alignItems:"center",gap:6}}>{p.customer}{p.isTest&&<TestPill/>}</div><div style={{color:C.light,fontSize:10}}>{p.postcode}</div></td>
                        <td style={{padding:"9px 13px",fontSize:12}}><div>📞 {p.phone||"—"}</div><div style={{color:C.mid}}>✉ {p.email||"—"}</div></td>
                        <td style={{padding:"9px 13px",fontSize:12}}>{p.appliance||<span style={{color:C.light}}>—</span>}</td>
                        <td style={{padding:"9px 13px",fontSize:12,fontWeight:600}}>{p.brand||<span style={{color:C.light}}>—</span>}</td>
                        <td style={{padding:"9px 13px",fontSize:11}}>{p.startedAt?new Date(p.startedAt).toLocaleDateString("en-GB"):"—"}</td>
                        <td style={{padding:"9px 13px",fontSize:12}}>{linkedJob?<button onClick={e=>{e.stopPropagation();setSelJob(linkedJob);}} style={{background:"none",border:"none",color:C.primary,fontWeight:800,cursor:"pointer",fontFamily:"inherit",padding:0,fontSize:12}}>{linkedJob.bookingRef||('#'+linkedJob.id)}</button>:<span style={{color:C.light}}>—</span>}</td>
                        <td style={{padding:"9px 13px"}}>
                          <span style={{background:p.status==="active"?C.purpleLight:"rgba(100,116,139,0.15)",color:p.status==="active"?C.purple:"#64748B",padding:"2px 9px",borderRadius:20,fontSize:11,fontWeight:700,display:"inline-flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}>
                            <span style={{width:6,height:6,borderRadius:"50%",background:p.status==="active"?C.purple:"#64748B",flexShrink:0}}/>{p.status==="active"?"Active":"Cancelled"}
                          </span>
                        </td>
                        <td style={{padding:"9px 13px",textAlign:"right"}} onClick={e=>e.stopPropagation()}>
                          {p.status==="active"
                            ? <Btn onClick={()=>setPlanStatus(p.id,"cancelled")} variant="ghost" sm>Cancel Plan</Btn>
                            : <Btn onClick={()=>setPlanStatus(p.id,"active")} variant="ghost" sm>Reactivate</Btn>}
                        </td>
                      </tr>;
                    })}
                  </tbody>
          ...
          {/* ── ENGINEERS ── */}
          {view==="engineers"&&(()=>{
            // Active-count badge shown next to each SP type in the side nav
            // below -- always computed across every SP of that type regardless
            // of which Active/Onboarding/Archive tab is currently selected.
            const typeCounts = Object.fromEntries(SP_TYPES.map(t=>
              [t.key, engineers.filter(eng=>(eng.type||"appliance")===t.key && engineerStatus(eng)==="active").length]
            ));
            const typeEngineers = engineers.filter(eng=>(eng.type||"appliance")===spType);
            const withStatus = typeEngineers.map(eng=>({eng,status:engineerStatus(eng)}));
            ...
            const activeType = SP_TYPES.find(t=>t.key===spType)||SP_TYPES[0];
            return (
            <div className="flex flex-col md:flex-row" style={{gap:20,alignItems:"flex-start"}}>
              {/* SP type side nav -- switches which SP type's list is shown
                  below. Badge to the left of each label is that type's
                  current Active count, always visible regardless of which
                  type is selected. On phones this becomes a horizontally
                  scrollable pill row above the content instead of a fixed
                  190px column beside it -- a fixed side column was leaving
                  the content area too narrow to be usable on a phone
                  viewport. */}
              <div className="w-full md:w-[190px] flex-row md:flex-col overflow-x-auto md:overflow-visible" style={{display:"flex",flexShrink:0,gap:4,WebkitOverflowScrolling:"touch"}}>
                {SP_TYPES.map(t=>(
                  <button key={t.key} onClick={()=>setSpType(t.key)} style={{
                    textAlign:"left",border:"none",cursor:"pointer",fontFamily:"inherit",
                    padding:"9px 12px",borderRadius:8,fontSize:13,fontWeight:800,
                    background:spType===t.key?C.primaryLight:"transparent",
                    color:spType===t.key?C.primary:C.mid,
                    display:"flex",alignItems:"center",gap:8,flexShrink:0,whiteSpace:"nowrap",
                  }}>
                    <span style={{
                      minWidth:20,height:20,padding:"0 5px",borderRadius:10,flexShrink:0,
                      display:"inline-flex",alignItems:"center",justifyContent:"center",
                      fontSize:11,fontWeight:800,
                      background:spType===t.key?C.primary:"rgba(255,255,255,0.08)",
                      color:spType===t.key?"#000":C.light,
                    }} title={`${typeCounts[t.key]||0} active`}>{typeCounts[t.key]||0}</span>
                    <span>{t.label}</span>
                  </button>
                ))}
                {/* External SP's -- a separate roster entirely (0007_external_sps.sql),
                    not one more trade type: an external SP is defined by the
                    business relationship (one-off, no Portal login), not by what
                    they repair, so it sits here as its own nav entry rather than
                    inside SP_TYPES. */}
                <div className="hidden md:block" style={{height:1,background:C.border,margin:"6px 4px"}}/>
                <button onClick={()=>setSpType("external")} style={{
                  textAlign:"left",border:"none",cursor:"pointer",fontFamily:"inherit",
                  padding:"9px 12px",borderRadius:8,fontSize:13,fontWeight:800,
                  background:spType==="external"?C.purpleLight:"transparent",
                  color:spType==="external"?C.purple:C.mid,
                  display:"flex",alignItems:"center",gap:8,flexShrink:0,whiteSpace:"nowrap",
                }}>
                  <span style={{
                    minWidth:20,height:20,padding:"0 5px",borderRadius:10,flexShrink:0,
                    display:"inline-flex",alignItems:"center",justifyContent:"center",
                    fontSize:11,fontWeight:800,
                    background:spType==="external"?C.purple:"rgba(255,255,255,0.08)",
                    color:spType==="external"?"#000":C.light,
                  }} title={`${activeExternalSps.length} active`}>{activeExternalSps.length}</span>
                  <span>External SP's</span>
                </button>
              </div>
              ...
              {spType==="external" ? (
                <ExternalSpsSection
                  externalSps={externalSps} extTab={extTab} onExtTabChange={setExtTab}
                  jobs={jobs} isOwner={isOwner} accent={C.purple}
                  onAdd={()=>setShowAddExternalSp(true)}
                  onEdit={setEditExternalSp}
                  onArchive={(sp)=>askArchiveExt(sp.id,sp.name)}
                  onRestore={(sp)=>restoreExternalSp(sp.id)}
                  onDelete={(sp)=>askDeleteExt(sp.id,sp.name)}
                  onOpenJob={setSelJob}
                />
              ) : (<>
              <div className="flex flex-col md:flex-row md:justify-between md:items-center" style={{gap:10,marginBottom:16}}>
                <h1 style={{margin:0,fontSize:19,fontWeight:900,color:C.text}}>SP's <span style={{color:C.light,fontWeight:600}}>— {activeType.label}</span></h1>
                <div className="flex" style={{gap:8,alignItems:"center"}}>
                  {engTab!=="archived"&&(
                    <div style={{display:"flex",gap:2,background:"rgba(255,255,255,0.06)",borderRadius:8,padding:2}}>
                      <button type="button" onClick={()=>setSpView("card")} title="Card view" style={{border:"none",cursor:"pointer",fontFamily:"inherit",borderRadius:6,padding:"6px 10px",fontSize:12,fontWeight:700,background:spView==="card"?C.primary:"transparent",color:spView==="card"?"#000":C.mid}}>▦ Cards</button>
                      <button type="button" onClick={()=>setSpView("list")} title="List view" style={{border:"none",cursor:"pointer",fontFamily:"inherit",borderRadius:6,padding:"6px 10px",fontSize:12,fontWeight:700,background:spView==="list"?C.primary:"transparent",color:spView==="list"?"#000":C.mid}}>☰ List</button>
                    </div>
                  )}
                  ...
                  {shown.map((eng,i)=>(
                    <div key={eng.id} style={{padding:"12px 18px",borderBottom:i<shown.length-1?`1px solid ${C.border}`:"none",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                      ...
                      <div style={{flex:1,minWidth:120}}>
                        ...
                        <div style={{color:C.light,fontSize:11}}><CallLink phone={eng.phone}/> · {eng.email}</div>
              ...
              <div style={spView==="list"
                ...
                {shown.map(eng=>{
                  ...
                  // ── LIST ROW — denser single-line-per-SP alternative to the
                  // card below, for scanning a large roster at a glance. Same
                  // click target (opens the jobs modal) and action buttons,
                  // just fewer details shown inline (postcode/appliance chips
                  // collapse to a count rather than listing every one).
                  if(spView==="list") return (
                    <div key={eng.id} onClick={()=>setSelEngJobs(eng)} title={`View all jobs assigned to ${eng.name}`} style={{background:C.card,borderRadius:11,border:`1px solid ${C.primary}`,boxShadow:"0 1px 3px rgba(0,0,0,.05)",padding:"10px 15px",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap",cursor:"pointer"}}>
                      <Av initials={eng.name.split(" ").map(w=>w[0]).join("")} color={accent} size={30}/>
                      <div style={{flex:"1 1 190px",minWidth:150}}>
                        <div style={{fontWeight:800,fontSize:13,display:"flex",alignItems:"center",gap:6}}>{eng.name}{eng.isTest&&<TestPill/>}</div>
                        <div style={{color:C.light,fontSize:11}}><CallLink phone={eng.phone}/> · {eng.email}</div>
                      </div>

                      {engTab==="onboarding" ? (
                        <div style={{flex:"0 0 auto",minWidth:130}}>
                          <div style={{fontSize:11,fontWeight:800,color:C.warn}}>Step {prog.completed} of {prog.total}</div>
                          {prog.next&&<div style={{fontSize:10,color:C.mid}}>Next: {prog.next.label}</div>}
                        </div>
                      ) : (
                        <div style={{display:"flex",gap:14,flex:"0 0 auto"}}>
                          {[["Active",active,C.blue],["Repaired",done,C.success],["BER",ber,C.danger],["Success",sr!==null?`${sr}%`:"—",sr>=80?C.success:sr>=60?C.warn:sr!==null?C.danger:C.light]].map(([l,v,col])=>(
                            <div key={l} style={{textAlign:"center"}}><div style={{fontWeight:900,fontSize:14,color:col}}>{v}</div><div style={{fontSize:8,color:C.light,textTransform:"uppercase",letterSpacing:.3}}>{l}</div></div>
                          ))}
                        </div>
                      )}

                      <div style={{flex:"1 1 150px",minWidth:120,fontSize:11,color:C.light}}>
                        {eng.postcodes.length?`${eng.postcodes.length} postcode area${eng.postcodes.length===1?"":"s"}`:<span style={{fontStyle:"italic"}}>No postcodes set</span>}
                        {" · "}
                        {eng.applianceTypes.length?`${eng.applianceTypes.length} appliance${eng.applianceTypes.length===1?"":"s"}`:<span style={{fontStyle:"italic"}}>No appliances set</span>}
                      </div>

                      {isOwner&&<span style={{fontWeight:800,color:accent,fontSize:12,flex:"0 0 auto"}}>£{eng.rate}/job</span>}
                      {isOwner&&owed>0&&<span style={{fontSize:11,color:"#f87171",fontWeight:700,flex:"0 0 auto"}}>£{owed} owed</span>}
                  ...
                  return (
                    <div key={eng.id} onClick={()=>setSelEngJobs(eng)} title={`View all jobs assigned to ${eng.name}`} style={{background:C.card,borderRadius:13,border:`1px solid ${C.primary}`,boxShadow:"0 1px 3px rgba(0,0,0,.05)",overflow:"hidden",cursor:"pointer"}}>
                      <div style={{padding:"13px 15px",borderBottom:`1px solid ${C.border}`,display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}}>
                        <Av initials={eng.name.split(" ").map(w=>w[0]).join("")} color={accent}/>
                        <div style={{flex:1,minWidth:120}}><div style={{fontWeight:800,fontSize:14,display:"flex",alignItems:"center",gap:6}}>{eng.name}{eng.isTest&&<TestPill/>}</div><div style={{color:C.light,fontSize:11}}><CallLink phone={eng.phone}/> · {eng.email}</div></div>
                        {isOwner&&<span style={{fontWeight:800,color:accent,fontSize:13}}>£{eng.rate}/job</span>}
                        <Btn onClick={e=>{e.stopPropagation();setEditEng(eng);}} variant="ghost" sm>Edit</Btn>
                        <Btn onClick={e=>{e.stopPropagation();askResetPassword(eng.profileId,eng.name);}} variant="ghost" sm>Reset Password</Btn>
                        <Btn onClick={e=>{e.stopPropagation();askArchive(eng.profileId,eng.name);}} variant="ghost" sm style={{color:C.danger}}>Archive</Btn>
                      </div>
                      ...
                      <div style={{padding:"8px 15px"}}>
                        {eJobs.filter(j=>!["Completed","Beyond Repair","Cancelled"].includes(j.status)).slice(0,2).map(j=>(
                          <div key={j.id} onClick={e=>{e.stopPropagation();setSelJob(j);}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",fontSize:12,padding:"4px 0",cursor:"pointer",color:C.mid}} onMouseOver={e=>e.currentTarget.style.color=C.primary} onMouseOut={e=>e.currentTarget.style.color=C.mid}>
                            <span><strong style={{color:C.text}}>{j.customer}</strong> · {j.appliance}{j.brand?` · ${j.brand}`:""}</span><Badge status={j.status}/>
                          </div>
                        ))}
                        ...
                        {eJobs.length>0&&<div style={{fontSize:11,color:C.primary,fontWeight:700,marginTop:6}}>View all {eJobs.length} job{eJobs.length===1?"":"s"} →</div>}
                      </div>
                    </div>
                  );
                })}
          ...
          {view==="payments"&&(()=>{
            const payees = [
              ...
              ...externalGroups.map((g,gi)=>{
                ...
                return {
                  key:`ext-${gi}`, external:true, externalId:g.id,
                ...
                <div key={p.key} style={{background:C.card,borderRadius:13,boxShadow:"0 1px 3px rgba(0,0,0,.05)",marginBottom:16,overflow:"hidden",...(p.external?{border:`1px solid ${C.purple}33`}:{})}}>
                  <div style={{padding:"13px 18px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:"#161B22"}}>
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:12}}>
                      {p.externalId&&<Btn onClick={()=>{setSpType("external");setExtTab("active");setView("engineers");const sp=externalSps.find(x=>x.id===p.externalId);if(sp)setEditExternalSp(sp);}} variant="ghost" sm>View SP</Btn>}
          ...
          {view==="settings"&&isOwner&&(
            <SettingsView pricing={pricing} onReload={loadPricing}
              ...
              onReloadJobs={loadBookings}
              onReloadEngineers={loadEngineers}
            />
          )}
      ...
      {(showNew||editJob)&&(
        <Modal title={editJob?`Edit Job ${editJob.bookingRef||('#'+editJob.id)}`:(newDefaultStatus==="Quote"?"New Quote":"New Booking")} onClose={()=>{setShowNew(false);setEditJob(null);}} wide>
          <JobForm initial={editJob} defaultStatus={newDefaultStatus} onSave={saveJob} onCancel={()=>{setShowNew(false);setEditJob(null);}} canEditRate={isOwner} engineers={assignableEngineers} jobs={jobs} externalSps={activeExternalSps}/>
        </Modal>
      )}
      ...
      {reassign&&(
        <ReassignModal job={reassign} engineers={assignableEngineers} jobs={jobs} externalSps={activeExternalSps} onReassign={doReassign} onClose={()=>setReassign(null)}/>
      )}
      {selPlan&&(
        <Modal title={`Protection Plan — ${selPlan.customer||"Unknown"}`} onClose={()=>setSelPlan(null)}>
          <PlanDetail plan={selPlan} linkedJob={jobs.find(j=>j.id===selPlan.bookingId)}
            onClose={()=>setSelPlan(null)}
            onCancelPlan={()=>{setPlanStatus(selPlan.id,"cancelled");setSelPlan(null);}}
            onReactivatePlan={()=>{setPlanStatus(selPlan.id,"active");setSelPlan(null);}}
            onOpenJob={job=>{setSelPlan(null);setSelJob(job);}}/>
        </Modal>
      )}
      {selEngJobs&&(
        <Modal title={`${selEngJobs.name} — All Jobs`} onClose={()=>setSelEngJobs(null)}>
          <EngineerJobsList eng={selEngJobs} jobs={jobs} onOpenJob={job=>{setSelEngJobs(null);setSelJob(job);}}/>
        </Modal>
      )}
      {editEng&&(
      ...
      {showAddEng&&(
        <AddEngineerModal defaultType={spType} onCreated={()=>{loadEngineers();setShowAddEng(false);}} onCancel={()=>setShowAddEng(false)}/>
      )}
      {showAddExternalSp&&(
        <AddExternalSpModal onCreated={()=>{loadExternalSps();setShowAddExternalSp(false);}} onCancel={()=>setShowAddExternalSp(false)}/>
      )}
      {editExternalSp&&(
        <Modal title={`Edit External SP: ${editExternalSp.name}`} onClose={()=>setEditExternalSp(null)} wide>
          <ExternalSpEditor sp={editExternalSp} onSave={saveExternalSp} onCancel={()=>setEditExternalSp(null)} isOwner={isOwner}/>
        </Modal>
      )}
      {extConfirm&&(
        <ConfirmModal
          title={extConfirm.kind==="archive"?`Archive ${extConfirm.label}?`:`Permanently delete ${extConfirm.label}?`}
          message={extConfirm.kind==="archive"
            ? "They'll be moved to the Archive tab and won't show up in the active roster or be assignable to new jobs. You can restore them from Archive at any time."
            : "This permanently deletes their external SP record. This cannot be undone -- any bookings they've already done keep their own snapshotted contact details regardless."}
          confirmLabel={extConfirm.kind==="archive"?"Archive":"Delete Forever"}
          danger={extConfirm.kind==="delete"}
          busy={extConfirmBusy}
          err={extConfirmErr}
          onConfirm={runExtConfirm}
          onCancel={()=>{setExtConfirm(null);setExtConfirmErr("");}}
        />
      )}
      {confirmModal&&(
        <ConfirmModal
